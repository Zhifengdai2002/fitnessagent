"""Coach tool execution, context building, and utilities.

Dead LLM-routing functions have been removed; the ReAct Coach node
(agent/nodes/coach.py) is the entry point for all chat interactions.
"""

from __future__ import annotations

import json
import re
from copy import deepcopy
from datetime import datetime, timedelta
from difflib import SequenceMatcher
from typing import Any

from agent.graph import run_agent
from agent.llm import call_model_json, load_prompt
from agent.services.memory import append_memory_item, default_memory_store, memory_context_for_planning, normalize_memory_store
from agent.services.persistence import PERSISTED_SESSION_KEYS, safe_iso_date
from agent.services.planning_helpers import (
    current_interaction_date,
    iso_to_date,
    same_iso_date,
    sort_workout_sessions,
)
from agent.services.state_builders import build_initial_state
from agent.state import FitnessAgentState
from agent.nodes.planner import (
    _build_session_blueprint,
    _finalize_workout_session,
    _map_goal_tag,
)
from agent.tools import (
    build_exercise_plan_payload,
    build_video_resources,
    calculate_food_macros,
    find_foods,
    get_food_by_name,
    search_similar_exercises,
)

ALLOWED_CHANGE_REQUEST_TYPES = {"workout_change", "nutrition_change", "mixed_change", "recovery_change", "none", "unclear"}
ALLOWED_CHANGE_REQUEST_SCOPES = {"today_only", "current_cycle", "future_default", "permanent", "unclear"}
ALLOWED_FOCUS_CATEGORIES = {
    "",
    "upper_chest_arms",
    "upper_shoulders",
    "back_training",
    "lower_legs_glutes",
    "functional_core",
    "functional_power",
    "functional_conditioning",
}
ALLOWED_NORMALIZED_INTENTS = {
    "no_action",
    "update_today_plan",
    "update_cycle_plan",
    "replace_exercise",
    "replace_food",
    "adjust_workout_volume",
    "adjust_sets",
    "adjust_intensity",
    "add_exercise",
    "remove_exercise",
    "cancel_workout",
    "answer_question",
    "unclear",
}

ALLOWED_COACH_TOOLS = {
    "no_action",
    "cancel_workout",
    "adjust_workout_volume",
    "replace_exercise",
    "replace_food",
    "update_today_plan",
    "update_cycle_plan",
    "write_memory",
}

COACH_NATIVE_TOOLS = [
    {
        "type": "function",
        "function": {
            "name": "cancel_workout",
            "description": "Cancel today's workout because the user requested cancellation or reported injury/pain.",
            "parameters": {
                "type": "object",
                "properties": {
                    "injury_reported": {"type": "boolean"},
                    "injury_areas": {"type": "array", "items": {"type": "string"}},
                    "reason": {"type": "string"},
                    "summary": {"type": "string"},
                },
                "required": [],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "adjust_workout_volume",
            "description": "Adjust today's workout volume. Use this for explicit set changes, adding/removing exercises, or vague higher/lower intensity. Preserve current focus.",
            "parameters": {
                "type": "object",
                "properties": {
                    "set_adjustment": {"type": "string", "enum": ["increase", "decrease", "target"]},
                    "set_target": {"type": "integer", "minimum": 0, "maximum": 12},
                    "exercise_count_adjustment": {"type": "string", "enum": ["increase", "decrease", "target"]},
                    "exercise_count_target": {"type": "integer", "minimum": 0, "maximum": 8},
                    "exercise_count_delta": {
                        "type": "integer",
                        "minimum": -8,
                        "maximum": 8,
                        "description": "Relative exercise-count change, e.g. +2 for 'add two exercises' or -1 for 'remove one exercise'.",
                    },
                    "intensity_adjustment": {"type": "string", "enum": ["higher", "lower"]},
                    "summary": {"type": "string"},
                },
                "required": [],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "replace_exercise",
            "description": (
                "Replace or remove an exercise from today's workout. "
                "To swap: set target to the exercise name. "
                "To remove without replacement: set target to the exercise name and remove_only to true."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "target": {"type": "string", "description": "Name of the exercise to replace or remove."},
                    "remove_only": {"type": "boolean", "description": "Set true when the user wants to remove the exercise without adding a replacement."},
                    "replacement_preference": {"type": "string"},
                    "summary": {"type": "string"},
                },
                "required": [],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "replace_food",
            "description": (
                "Remove one or more food items from today's nutrition and replace them with alternatives "
                "chosen automatically by the system. "
                "Put the food name(s) the user wants removed into temporary_food_avoidances — "
                "do NOT ask the user what to replace them with. "
                "The system picks suitable substitutes; report the actual substitution from the tool result."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "temporary_food_avoidances": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Food names to remove, e.g. ['apple'] when user says 'change apple'.",
                    },
                    "summary": {"type": "string"},
                },
                "required": ["temporary_food_avoidances"],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "update_today_plan",
            "description": "Use the planner workflow to update today's workout focus or broader same-day plan.",
            "parameters": {
                "type": "object",
                "properties": {
                    "focus_category": {
                        "type": "string",
                        "enum": [
                            "",
                            "upper_chest_arms",
                            "upper_shoulders",
                            "back_training",
                            "lower_legs_glutes",
                            "functional_core",
                            "functional_power",
                            "functional_conditioning",
                        ],
                    },
                    "summary": {"type": "string"},
                },
                "required": [],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "update_cycle_plan",
            "description": "Modify a non-today session in the current training cycle. Use for requests that name another day/date, tomorrow, next session, weekly plan, or current cycle.",
            "parameters": {
                "type": "object",
                "properties": {
                    "target_date": {"type": "string"},
                    "target_day": {"type": "string"},
                    "target_session": {"type": "string"},
                    "cycle_operation": {
                        "type": "string",
                        "enum": ["replace_focus", "replace_exercise", "adjust_volume", "cancel_session"],
                    },
                    "focus_category": {
                        "type": "string",
                        "enum": [
                            "",
                            "upper_chest_arms",
                            "upper_shoulders",
                            "back_training",
                            "lower_legs_glutes",
                            "functional_core",
                            "functional_power",
                            "functional_conditioning",
                        ],
                    },
                    "target": {"type": "string"},
                    "summary": {"type": "string"},
                },
                "required": [],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "write_memory",
            "description": (
                "Persist something the user said that should be remembered in future sessions. "
                "Call this when the user mentions a preference, dislike, injury, or personal metric "
                "that is NOT already captured by another tool in the same turn. "
                "Examples: 'I hate burpees', 'I'm vegetarian', 'I weigh 72 kg now', 'my knee has been bothering me'."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "collection": {
                        "type": "string",
                        "enum": [
                            "injury_events",
                            "food_preferences",
                            "training_preferences",
                            "exercise_feedback_records",
                            "body_metrics",
                        ],
                        "description": (
                            "injury_events — pain, strain, injury. "
                            "food_preferences — food likes/dislikes/avoidances. "
                            "training_preferences — exercise/focus preferences. "
                            "exercise_feedback_records — feedback on a specific exercise. "
                            "body_metrics — weight, body fat, or other measurements."
                        ),
                    },
                    "data": {
                        "type": "object",
                        "description": "The record to store. Include relevant fields for the collection (e.g. areas, reason, active for injury_events; food, scope for food_preferences).",
                    },
                    "note": {"type": "string", "description": "Optional one-line human-readable summary of why this was saved."},
                },
                "required": ["collection", "data"],
            },
        },
    },
]








def execute_coach_tool_call(tool_call: dict[str, Any], context: dict[str, Any]) -> str:
    tool_name = str(tool_call.get("tool_name", "no_action")).strip()
    tool = coach_tool_registry().get(tool_name)
    if not tool:
        return ""
    return str(tool["handler"](context, dict(tool_call.get("arguments", {}))) or "")


def coach_tool_registry() -> dict[str, dict[str, Any]]:
    return {
        "no_action": {
            "description": "No app state change.",
            "handler": handle_no_action_tool,
        },
        "cancel_workout": {
            "description": "Cancel today's workout.",
            "handler": handle_cancel_workout_tool,
        },
        "adjust_workout_volume": {
            "description": "Adjust today's sets, reps, notes, or exercise count.",
            "handler": handle_adjust_workout_volume_tool,
        },
        "replace_exercise": {
            "description": "Replace same-focus exercises.",
            "handler": handle_replace_exercise_tool,
        },
        "replace_food": {
            "description": "Replace today's food.",
            "handler": handle_replace_food_tool,
        },
        "update_today_plan": {
            "description": "Patch today's workout plan.",
            "handler": handle_update_today_plan_tool,
        },
        "update_cycle_plan": {
            "description": "Patch a non-today session in the current training cycle.",
            "handler": handle_update_cycle_plan_tool,
        },
        "write_memory": {
            "description": "Persist a user preference, injury, or metric for future sessions.",
            "handler": handle_write_memory_tool,
        },
    }


def fallback_tool_call_from_decision(decision: dict[str, Any]) -> dict[str, Any]:
    route = str(decision.get("route", "none"))
    normalized = dict(decision.get("normalized", {}))
    action = str(decision.get("planner_action", ""))
    if route == "none":
        return {"tool_name": "no_action", "arguments": {}, "source": "fallback"}
    if route == "safety" or normalized.get("cancel_today") or normalized.get("injury_reported"):
        return {"tool_name": "cancel_workout", "arguments": normalized, "source": "fallback"}
    if action == "update_cycle_plan" or str(normalized.get("intent", "")) == "update_cycle_plan":
        return {"tool_name": "update_cycle_plan", "arguments": normalized, "source": "fallback"}
    if action == "replace_exercise":
        return {"tool_name": "replace_exercise", "arguments": normalized, "source": "fallback"}
    if action == "adjust_workout_volume" or workout_volume_change_present(normalized):
        return {"tool_name": "adjust_workout_volume", "arguments": normalized, "source": "fallback"}
    if action == "replace_food" or coerce_string_list(normalized.get("temporary_food_avoidances"), []):
        return {"tool_name": "replace_food", "arguments": normalized, "source": "fallback"}
    return {"tool_name": "update_today_plan", "arguments": normalized, "source": "fallback"}


def sanitize_tool_call(planned: dict[str, Any], fallback: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(planned, dict):
        return fallback
    tool_name = str(planned.get("tool_name", "")).strip()
    if tool_name not in ALLOWED_COACH_TOOLS:
        return fallback
    if str(fallback.get("tool_name", "no_action")) != "no_action" and tool_name == "no_action":
        return fallback
    confidence = clamp_float(planned.get("confidence"), 0.0, 1.0, 0.0)
    if confidence < 0.45:
        return fallback
    fallback_tool = str(fallback.get("tool_name", "no_action"))
    if fallback_tool in {"cancel_workout", "adjust_workout_volume", "replace_exercise", "replace_food", "update_cycle_plan"}:
        tool_name = fallback_tool
    arguments = dict(fallback.get("arguments", {}))
    planned_arguments = planned.get("arguments", {})
    if isinstance(planned_arguments, dict):
        arguments.update(planned_arguments)
    return {
        "tool_name": tool_name,
        "arguments": sanitize_tool_arguments(tool_name, arguments),
        "reason": str(planned.get("summary") or planned.get("reason") or "").strip(),
        "confidence": confidence,
        "source": "planner_agent",
    }


def sanitize_tool_arguments(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    sanitized = sanitize_normalized_change_request(arguments)
    if tool_name == "adjust_workout_volume":
        set_adjustment = str(arguments.get("set_adjustment") or sanitized.get("set_adjustment") or "").strip()
        if set_adjustment not in {"increase", "decrease", "target"}:
            set_adjustment = ""
        sanitized["set_adjustment"] = set_adjustment
        sanitized["set_target"] = clamp_int(arguments.get("set_target") or sanitized.get("set_target"), 0, 12, 0)
        exercise_count_adjustment = str(
            arguments.get("exercise_count_adjustment") or sanitized.get("exercise_count_adjustment") or ""
        ).strip()
        if exercise_count_adjustment not in {"increase", "decrease", "target"}:
            exercise_count_adjustment = ""
        sanitized["exercise_count_adjustment"] = exercise_count_adjustment
        sanitized["exercise_count_target"] = clamp_int(
            arguments.get("exercise_count_target") or sanitized.get("exercise_count_target"),
            0,
            8,
            0,
        )
        exercise_count_delta = clamp_int(
            arguments.get("exercise_count_delta") or sanitized.get("exercise_count_delta"),
            -8,
            8,
            0,
        )
        sanitized["exercise_count_delta"] = exercise_count_delta
        if not exercise_count_adjustment and exercise_count_delta:
            sanitized["exercise_count_adjustment"] = "increase" if exercise_count_delta > 0 else "decrease"
        intensity = str(
            arguments.get("intensity_adjustment")
            or arguments.get("intensity")
            or sanitized.get("intensity_adjustment")
            or ""
        ).strip()
        sanitized["intensity_adjustment"] = intensity if intensity in {"higher", "lower"} else ""
        sanitized["intent"] = "adjust_workout_volume"
    elif tool_name == "cancel_workout":
        if not sanitized.get("recovery_signal_only"):
            sanitized["cancel_today"] = True
    elif tool_name == "update_cycle_plan":
        operation = str(
            arguments.get("cycle_operation")
            or arguments.get("operation")
            or sanitized.get("cycle_operation")
            or ""
        ).strip()
        if operation not in {"replace_focus", "replace_exercise", "adjust_volume", "cancel_session"}:
            if sanitized.get("cancel_today") or sanitized.get("injury_reported"):
                operation = "cancel_session"
            elif sanitized.get("focus_category"):
                operation = "replace_focus"
            elif sanitized.get("intent") == "replace_exercise":
                operation = "replace_exercise"
            elif workout_volume_change_present(sanitized):
                operation = "adjust_volume"
            else:
                operation = "replace_exercise"
        sanitized["cycle_operation"] = operation
        sanitized["target_date"] = str(arguments.get("target_date") or sanitized.get("target_date") or "").strip()
        sanitized["target_day"] = str(arguments.get("target_day") or sanitized.get("target_day") or "").strip()
        sanitized["target_session"] = str(arguments.get("target_session") or sanitized.get("target_session") or "").strip()
        sanitized["intent"] = "update_cycle_plan"
    elif tool_name == "write_memory":
        sanitized = {
            "collection": str(arguments.get("collection") or "").strip(),
            "data": arguments.get("data") if isinstance(arguments.get("data"), dict) else {},
            "note": str(arguments.get("note") or "").strip(),
        }
    return sanitized




def handle_no_action_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    return ""


def handle_cancel_workout_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    return cancel_today_from_chat(arguments, context["previous_result"], context["session_state"])


def handle_adjust_workout_volume_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    normalized = sanitize_tool_arguments("adjust_workout_volume", arguments)
    return adjust_today_workout_volume_from_chat(normalized, context["previous_result"], context["session_state"])


def handle_replace_exercise_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    # Inject intent so replace_today_exercise_from_chat's string-matching guard is bypassed —
    # if the LLM called this tool, trust its decision unconditionally.
    return replace_today_exercise_from_chat(
        context["user_message"],
        context["previous_result"],
        context["session_state"],
        {**arguments, "intent": "replace_exercise"},
    )


def handle_replace_food_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    return update_nutrition_from_chat(arguments, context["previous_result"], context["session_state"])


def handle_update_today_plan_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    return execute_ai_plan_patch(
        user_message=context["user_message"],
        profile_inputs=context["profile_inputs"],
        previous_result=context["previous_result"],
        normalized=arguments,
        session_state=context["session_state"],
    )


def handle_update_cycle_plan_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    return update_cycle_plan_from_chat(
        user_message=context["user_message"],
        previous_result=context["previous_result"],
        session_state=context["session_state"],
        arguments=arguments,
    )


def handle_write_memory_tool(context: dict[str, Any], arguments: dict[str, Any]) -> str:
    from datetime import date
    from agent.services.memory import append_memory_item, normalize_memory_store

    collection = str(arguments.get("collection") or "").strip()
    data = arguments.get("data")
    if not collection or not isinstance(data, dict) or not data:
        return "Memory not saved: missing collection or data."

    session_state = context["session_state"]
    memory_store = normalize_memory_store(session_state.get("memory_store", {}))
    if "date" not in data:
        data = {**data, "date": str(date.today())}
    memory_store = append_memory_item(memory_store, collection, data)
    session_state["memory_store"] = memory_store
    note = str(arguments.get("note") or "").strip()
    return f"Saved to {collection}." + (f" ({note})" if note else "")




def fallback_coordinator_decision(user_message: str, result: FitnessAgentState | dict[str, Any]) -> dict[str, Any]:
    normalized = normalize_change_request(user_message)
    normalized = augment_chat_cycle_change(user_message, normalized, result)
    normalized = augment_chat_safety_change(user_message, normalized)
    normalized = augment_chat_workout_volume_change(user_message, normalized)
    normalized = augment_chat_food_change(user_message, normalized, result)
    if chat_message_is_advice_only_question(user_message, normalized):
        normalized = {**normalized, "intent": "answer_question", "request_type": "none", "scope": "unclear"}
        return {"route": "none", "normalized": normalized}
    normalized = augment_chat_focus_change(user_message, normalized)
    normalized = infer_missing_intent_from_normalized(normalized)

    intent = str(normalized.get("intent", "")).strip()
    if intent == "update_cycle_plan" or cycle_change_target_present(normalized):
        return {
            "route": "planner",
            "planner_action": "update_cycle_plan",
            "normalized": normalized,
            "action_message": "Training cycle was updated by AI Coach.",
        }
    if intent == "replace_food" or coerce_string_list(normalized.get("temporary_food_avoidances"), []):
        return {
            "route": "planner",
            "planner_action": "replace_food",
            "normalized": normalized,
            "action_message": "Today's nutrition was updated by AI Coach.",
        }
    if intent == "replace_exercise" or chat_message_requests_exercise_replacement(user_message):
        return {
            "route": "planner",
            "planner_action": "replace_exercise",
            "normalized": normalized,
            "action_message": "Today's plan was updated by AI Coach.",
        }
    if intent in {"adjust_workout_volume", "adjust_sets", "adjust_intensity", "add_exercise", "remove_exercise"} or workout_volume_change_present(normalized):
        return {
            "route": "planner",
            "planner_action": "adjust_workout_volume",
            "normalized": normalized,
            "action_message": "Today's plan was updated by AI Coach.",
        }
    if intent == "cancel_workout" or normalized.get("cancel_today") or normalized.get("injury_reported"):
        return {"route": "safety", "planner_action": "cancel_workout", "normalized": normalized}
    if intent == "update_today_plan":
        return {
            "route": "planner",
            "planner_action": "update_today_plan",
            "normalized": normalized,
            "action_message": "Today's plan was updated by AI Coach.",
        }
    if not chat_request_should_update_today(user_message, normalized):
        return {"route": "none", "normalized": normalized}
    return {
        "route": "planner",
        "planner_action": "update_today_plan",
        "normalized": normalized,
        "action_message": "Today's plan was updated by AI Coach.",
    }






def cancel_today_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    recovery_signal_only = bool(normalized.get("recovery_signal_only"))
    injury_reported = bool(normalized.get("injury_reported"))
    if not normalized.get("cancel_today") and not (injury_reported and recovery_signal_only):
        return ""

    injury_areas = coerce_string_list(normalized.get("injury_areas"), [])
    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    updated_result = deepcopy(previous_result)
    sessions = updated_result.get("current_plan", {}).get("workout_sessions", [])
    today_session = select_today_session(sort_workout_sessions(sessions), current_date)
    should_cancel_today = bool(normalized.get("cancel_today"))
    if injury_reported and recovery_signal_only and today_session:
        should_cancel_today = session_stresses_injury_area(today_session, injury_areas)
    elif injury_reported and not recovery_signal_only:
        should_cancel_today = True

    if should_cancel_today and not today_session:
        parsed_date = iso_to_date(current_date)
        sessions.insert(
            0,
            {
                "day": parsed_date.strftime("%A"),
                "scheduled_date": current_date,
                "cycle_number": updated_result.get("current_plan", {}).get("cycle_number", 1),
                "cycle_session_index": 0,
                "is_ad_hoc": True,
                "is_cancelled": True,
                "focus": "Workout Cancelled",
                "duration_minutes": 0,
                "warmup": [],
                "exercises": [],
                "cooldown": [],
                "safety_notes": cancel_safety_notes(injury_reported=injury_reported, injury_areas=injury_areas),
                "injury_reported": injury_reported,
                "injury_areas": injury_areas,
            },
        )
    elif should_cancel_today and today_session:
        today_session["is_cancelled"] = True
        today_session["focus"] = "Workout Cancelled"
        today_session["duration_minutes"] = 0
        today_session["warmup"] = []
        today_session["exercises"] = []
        today_session["cooldown"] = []
        today_session["safety_notes"] = cancel_safety_notes(injury_reported=injury_reported, injury_areas=injury_areas)
        today_session["injury_reported"] = injury_reported
        today_session["injury_areas"] = injury_areas

    protected_future_sessions: list[str] = []
    if injury_reported:
        protected_future_sessions = protect_future_sessions_for_injury(
            updated_result=updated_result,
            current_date=current_date,
            injury_areas=injury_areas,
        )
        record_memory_injury_event(
            session_state=session_state,
            date_iso=current_date,
            injury_areas=injury_areas,
            source="ai_coach",
            summary=f"User reported injury around {', '.join(injury_areas) if injury_areas else 'an unspecified area'}.",
            risk_level="medium",
        )
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type="cancel_workout" if not injury_reported else ("injury_cancel_workout" if should_cancel_today else "injury_cycle_protection"),
        summary=(
            "Cancelled today's workout after AI Coach safety routing."
            if should_cancel_today
            else "Recorded body-area discomfort and protected related future cycle sessions."
        ),
        injury_areas=injury_areas if injury_reported else [],
    )
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    if injury_reported:
        area_text = f" around {', '.join(injury_areas)}" if injury_areas else ""
        future_text = ""
        if protected_future_sessions:
            future_text = f" Related future sessions were protected: {', '.join(protected_future_sessions)}."
        if not should_cancel_today:
            unchanged_text = " Today's workout was left unchanged because it does not appear to target that area."
            return f"I recorded discomfort{area_text}.{future_text}{unchanged_text}"
        return f"Today's workout has been cancelled because you reported an injury{area_text}.{future_text}"
    return "Today's workout has been cancelled as requested. No injury reason was recorded."


def adjust_today_workout_volume_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    if normalized.get("focus_category") or normalized.get("cancel_today") or normalized.get("injury_reported"):
        return ""

    summaries: list[str] = []
    working_result: FitnessAgentState | dict[str, Any] = previous_result
    if str(normalized.get("set_adjustment", "")).strip():
        summary = patch_today_sets_from_chat(normalized, working_result, session_state)
        if summary:
            summaries.append(summary)
            working_result = session_state.get("agent_result", working_result)

    exercise_adjustment = str(normalized.get("exercise_count_adjustment", "")).strip().lower()
    if exercise_adjustment:
        summary = patch_today_exercise_count_from_chat(normalized, working_result, session_state)
        if summary:
            summaries.append(summary)
            working_result = session_state.get("agent_result", working_result)

    intensity = str(normalized.get("intensity_adjustment", "")).strip().lower()
    if intensity and not exercise_adjustment and not str(normalized.get("set_adjustment", "")).strip():
        summary = patch_today_intensity_from_chat(normalized, working_result, session_state)
        if summary:
            summaries.append(summary)

    return " ".join(summaries)


def patch_today_intensity_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    intensity = str(normalized.get("intensity_adjustment", "")).strip().lower()
    if intensity not in {"higher", "lower"}:
        return ""
    if normalized.get("focus_category") or normalized.get("cancel_today") or normalized.get("injury_reported"):
        return ""

    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    updated_result = deepcopy(previous_result)
    today_session = select_today_session(
        sort_workout_sessions(updated_result.get("current_plan", {}).get("workout_sessions", [])),
        current_date,
    )
    if not today_session or today_session.get("is_cancelled") or not today_session.get("exercises"):
        session_state["agent_result"] = updated_result
        return "Today has no active workout to adjust, so the current plan was left unchanged."

    fitness_level = str(previous_result.get("user_profile", {}).get("fitness_level", "intermediate")).strip().lower()
    updated_exercises = patch_exercises_for_intensity(
        exercises=list(today_session.get("exercises", [])),
        fitness_level=fitness_level,
        intensity=intensity,
        focus=focus_tag_from_session(today_session),
    )
    if not updated_exercises:
        return ""

    today_session["exercises"] = updated_exercises
    today_session.pop("is_cancelled", None)
    today_session["duration_minutes"] = int(today_session.get("duration_minutes") or 60)
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type=f"{intensity}_intensity",
        summary=f"Adjusted today's workout to {intensity} intensity.",
    )

    label = "lower" if intensity == "lower" else "higher"
    exercise_text = ", ".join(
        f"{exercise.get('name', '')} {exercise.get('sets', '')}x{exercise.get('reps', '')}".strip()
        for exercise in updated_exercises
    )
    return f"Today's workout was adjusted to {label} intensity: {exercise_text}."


def add_today_exercise_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    updated_result = deepcopy(previous_result)
    today_session = select_today_session(
        sort_workout_sessions(updated_result.get("current_plan", {}).get("workout_sessions", [])),
        current_date,
    )
    if not today_session or today_session.get("is_cancelled") or not today_session.get("exercises"):
        return "Today has no active workout to add an exercise to, so the current plan was left unchanged."

    focus = focus_tag_from_session(today_session)
    if not focus:
        return ""
    exercises = list(today_session.get("exercises", []))
    current_names = [
        str(exercise.get("name", "")).strip()
        for exercise in exercises
        if str(exercise.get("name", "")).strip()
    ]
    if not current_names:
        return ""

    fitness_level = str(previous_result.get("user_profile", {}).get("fitness_level", "intermediate")).strip().lower()
    additions: list[dict[str, Any]] = []
    for seed_name in current_names:
        additions = search_similar_exercises(
            exercise_name=seed_name,
            focus=focus,
            level=fitness_level,
            exclude=current_names,
            limit=8,
        )
        additions = same_focus_replacement_candidates(additions, focus)
        if additions:
            break
    if not additions:
        return ""

    set_policy = set_policy_from_exercises(exercises)
    rep_policy = rep_policy_from_exercises(exercises)
    added = replacement_exercise_payload({"sets": set_policy, "reps": rep_policy}, additions[0])
    exercises.append(added)
    today_session["exercises"] = exercises
    today_session.pop("is_cancelled", None)
    today_session["duration_minutes"] = int(today_session.get("duration_minutes") or 60)
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type="add_exercise",
        summary=f"Added {added.get('name', '')} to today's same-focus workout.",
    )
    return (
        f"Today's workout now has one more same-focus exercise: "
        f"{added.get('name', '')} {added.get('sets', '')}x{added.get('reps', '')}. "
        "Existing exercises kept their sets and reps."
    )


def patch_today_exercise_count_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    adjustment = str(normalized.get("exercise_count_adjustment", "")).strip().lower()
    if adjustment not in {"increase", "decrease", "target"}:
        return ""
    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    today_session = select_today_session(
        sort_workout_sessions(previous_result.get("current_plan", {}).get("workout_sessions", [])),
        current_date,
    )
    exercises = list(today_session.get("exercises", [])) if today_session else []
    current_count = len(exercises)
    delta = clamp_int(normalized.get("exercise_count_delta"), -8, 8, 0)
    if delta:
        target_count = max(2, min(8, current_count + delta))
        if target_count > current_count:
            summary = ""
            working_result = previous_result
            for _ in range(target_count - current_count):
                summary = add_today_exercise_from_chat(normalized, working_result, session_state)
                working_result = session_state.get("agent_result", working_result)
            added_count = target_count - current_count
            return (
                f"Today's workout now has {target_count} exercises after adding "
                f"{added_count} same-focus exercise{'s' if added_count != 1 else ''}. "
                "Existing exercises kept their sets and reps."
            ) if summary else "No same-focus exercise was available to add."
        if target_count < current_count:
            return remove_today_exercise_from_chat(
                {**normalized, "exercise_count_target": target_count},
                previous_result,
                session_state,
            )
        return "Today's workout is already at the exercise-count boundary."
    if adjustment == "target":
        target_count = clamp_int(normalized.get("exercise_count_target"), 2, 8, current_count)
        if target_count > current_count:
            summary = ""
            working_result = previous_result
            for _ in range(target_count - current_count):
                summary = add_today_exercise_from_chat(normalized, working_result, session_state)
                working_result = session_state.get("agent_result", working_result)
            return summary
        if target_count < current_count:
            return remove_today_exercise_from_chat({**normalized, "exercise_count_target": target_count}, previous_result, session_state)
        return "Today's workout already matches the requested exercise count."
    if adjustment == "increase":
        return add_today_exercise_from_chat(normalized, previous_result, session_state)
    return remove_today_exercise_from_chat(normalized, previous_result, session_state)


def remove_today_exercise_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    updated_result = deepcopy(previous_result)
    today_session = select_today_session(
        sort_workout_sessions(updated_result.get("current_plan", {}).get("workout_sessions", [])),
        current_date,
    )
    exercises = list(today_session.get("exercises", [])) if today_session else []
    if not today_session or today_session.get("is_cancelled") or not exercises:
        return "Today has no active workout to reduce, so the current plan was left unchanged."
    target_count = clamp_int(normalized.get("exercise_count_target"), 2, 8, 0)
    if not target_count:
        target_count = max(2, len(exercises) - 1)
    if len(exercises) <= target_count:
        return "Today's workout is already at the requested exercise-count boundary."
    removed = exercises[target_count:]
    today_session["exercises"] = exercises[:target_count]
    today_session.pop("is_cancelled", None)
    today_session["duration_minutes"] = int(today_session.get("duration_minutes") or 60)
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type="reduce_exercise_count",
        summary=f"Removed {', '.join(str(exercise.get('name', '')) for exercise in removed)} from today's workout.",
    )
    kept_text = ", ".join(
        f"{exercise.get('name', '')} {exercise.get('sets', '')}x{exercise.get('reps', '')}".strip()
        for exercise in today_session["exercises"]
    )
    return f"Today's workout exercise count was reduced while keeping the same focus: {kept_text}."


def patch_today_sets_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    set_adjustment = str(normalized.get("set_adjustment", "")).strip().lower()
    if set_adjustment not in {"increase", "decrease", "target"}:
        return ""
    if normalized.get("focus_category") or normalized.get("cancel_today") or normalized.get("injury_reported"):
        return ""

    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    updated_result = deepcopy(previous_result)
    today_session = select_today_session(
        sort_workout_sessions(updated_result.get("current_plan", {}).get("workout_sessions", [])),
        current_date,
    )
    if not today_session or today_session.get("is_cancelled") or not today_session.get("exercises"):
        session_state["agent_result"] = updated_result
        return "Today has no active workout to adjust, so the current plan was left unchanged."

    raw_target = clamp_int(normalized.get("set_target"), 0, 5, 0)
    target = max(3, raw_target) if raw_target else 0
    updated_exercises: list[dict[str, Any]] = []
    changed = False
    for exercise in today_session.get("exercises", []):
        patched = dict(exercise)
        current_sets = clamp_int(patched.get("sets"), 3, 5, 4)
        if set_adjustment == "increase":
            next_sets = min(5, current_sets + 1)
            note = "Set volume increased: keep the extra set crisp and stop if form drops."
        elif set_adjustment == "decrease":
            next_sets = max(3, current_sets - 1)
            note = "Set volume reduced: keep every set controlled and leave reps in reserve."
        else:
            next_sets = target or current_sets
            note = "Set volume adjusted by request: keep movement quality consistent across all sets."
        changed = changed or next_sets != current_sets
        patched["sets"] = next_sets
        existing_note = str(patched.get("notes", "")).strip()
        if note not in existing_note:
            patched["notes"] = f"{existing_note} {note}".strip()
        updated_exercises.append(patched)

    if not updated_exercises:
        return ""
    today_session["exercises"] = updated_exercises
    today_session.pop("is_cancelled", None)
    today_session["duration_minutes"] = int(today_session.get("duration_minutes") or 60)
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    action_type = f"sets_{set_adjustment}"
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type=action_type,
        summary="Adjusted today's workout sets without changing exercise count.",
    )

    exercise_text = ", ".join(
        f"{exercise.get('name', '')} {exercise.get('sets', '')}x{exercise.get('reps', '')}".strip()
        for exercise in updated_exercises
    )
    if not changed:
        return f"Today's workout already matches the requested set boundary: {exercise_text}."
    return f"Today's workout sets were updated without adding exercises: {exercise_text}."


def update_nutrition_from_chat(
    normalized: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    avoidances = coerce_string_list(normalized.get("temporary_food_avoidances"), [])
    if not avoidances or normalized_request_has_workout_change(normalized):
        return ""

    updated_result = deepcopy(previous_result)
    current_plan = updated_result.get("current_plan", {})
    meals = list(current_plan.get("meal_suggestions", []))
    if not meals:
        return ""

    updated_meals: list[dict[str, Any]] = []
    replacements: list[tuple[str, str]] = []
    used_foods = {
        str(meal.get("food_name", "")).strip()
        for meal in meals
        if str(meal.get("food_name", "")).strip()
    }
    for meal in meals:
        food_name = str(meal.get("food_name", "")).strip()
        if not food_matches_any_avoidance(food_name, avoidances):
            updated_meals.append(meal)
            continue
        replacement = replacement_food_for_meal(meal=meal, avoidances=avoidances, used_foods=used_foods)
        if replacement:
            replacement_name = str(replacement.get("food_name", "")).strip()
            replacements.append((food_name, replacement_name))
            used_foods.add(replacement_name)
            updated_meals.append(replacement)

    if not replacements:
        return ""

    current_plan["meal_suggestions"] = updated_meals
    session_state["agent_result"] = updated_result
    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    record_memory_food_avoidance(session_state=session_state, date_iso=current_date, avoidances=avoidances, scope="today_only")
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type="replace_food",
        summary=f"Replaced today's foods: {', '.join(old for old, _ in replacements)}.",
    )
    replacement_text = ", ".join(f"{old} -> {new}" for old, new in replacements)
    return f"Today's nutrition was updated: {replacement_text}. Your workout plan was left unchanged."


def replace_today_exercise_from_chat(
    user_message: str,
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
    arguments: dict[str, Any] | None = None,
) -> str:
    arguments = arguments or {}
    is_replace_intent = str(arguments.get("intent", "")) == "replace_exercise"
    if not chat_message_requests_exercise_replacement(user_message) and not is_replace_intent:
        return ""

    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    current_plan = previous_result.get("current_plan", {})
    today_session = select_today_session(sort_workout_sessions(current_plan.get("workout_sessions", [])), current_date)
    if not today_session or today_session.get("is_cancelled"):
        return ""

    exercises = today_session.get("exercises", [])
    target_text = str(arguments.get("target", "")).strip()
    target_index = exercise_index_from_chat_message(target_text or user_message, exercises)
    if target_index is None and not (is_replace_intent or chat_requests_general_exercise_replacement(user_message)):
        return ""

    updated_result = deepcopy(previous_result)
    updated_sessions = updated_result.get("current_plan", {}).get("workout_sessions", [])
    updated_today = select_today_session(sort_workout_sessions(updated_sessions), current_date)
    if not updated_today:
        return ""

    updated_exercises = list(updated_today.get("exercises", []))
    current_names = [str(exercise.get("name", "")).strip() for exercise in updated_exercises if str(exercise.get("name", "")).strip()]
    focus_tag = focus_tag_from_session(today_session)
    if not focus_tag:
        return ""
    level = str(previous_result.get("user_profile", {}).get("fitness_level", ""))

    if target_index is None:
        replacements_made = []
        excluded_names = list(current_names)
        for index, original_exercise in enumerate(updated_exercises):
            original_name = str(original_exercise.get("name", "")).strip()
            if not original_name:
                continue
            replacements = search_similar_exercises(
                exercise_name=original_name,
                focus=focus_tag,
                level=level,
                exclude=excluded_names,
                limit=5,
            )
            replacements = same_focus_replacement_candidates(replacements, focus_tag)
            if not replacements:
                continue
            replacement = replacements[0]
            updated_exercises[index] = replacement_exercise_payload(original_exercise, replacement)
            replacement_name = str(replacement.get("name", "")).strip()
            excluded_names.append(replacement_name)
            replacements_made.append((original_name, replacement_name))
        if not replacements_made:
            return ""
        updated_today["exercises"] = updated_exercises
        refresh_youtube_resources(updated_result)
        session_state["agent_result"] = updated_result
        record_memory_plan_modification(
            session_state=session_state,
            date_iso=current_date,
            action_type="replace_exercise",
            summary="Replaced today's exercises with same-focus alternatives.",
        )
        replacement_text = ", ".join(f"{old} -> {new}" for old, new in replacements_made)
        return f"Today's plan is now updated with same-focus exercise alternatives: {replacement_text}. Sets and reps stayed the same."

    original_exercise = updated_exercises[target_index]
    original_name = str(original_exercise.get("name", "")).strip()
    if not original_name:
        return ""

    if arguments.get("remove_only"):
        updated_exercises.pop(target_index)
        updated_today["exercises"] = updated_exercises
        refresh_youtube_resources(updated_result)
        session_state["agent_result"] = updated_result
        record_memory_plan_modification(
            session_state=session_state,
            date_iso=current_date,
            action_type="replace_exercise",
            summary=f"Removed {original_name} from today's workout.",
        )
        remaining = [str(e.get("name", "")) for e in updated_exercises if e.get("name")]
        return f"Removed {original_name}. Remaining exercises: {', '.join(remaining)}."

    replacements = search_similar_exercises(
        exercise_name=original_name,
        focus=focus_tag,
        level=level,
        exclude=current_names,
        limit=5,
    )
    replacements = same_focus_replacement_candidates(replacements, focus_tag)
    if not replacements:
        return ""

    replacement = replacements[0]
    updated_exercises[target_index] = replacement_exercise_payload(original_exercise, replacement)
    updated_today["exercises"] = updated_exercises
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type="replace_exercise",
        summary=f"Replaced {original_name} with {replacement.get('name', '')}.",
    )
    return f"Today's plan is now updated: replaced {original_name} with {replacement.get('name', '')} while keeping the same focus, sets, and reps."


def update_cycle_plan_from_chat(
    *,
    user_message: str,
    previous_result: FitnessAgentState | dict[str, Any],
    session_state: dict[str, Any],
    arguments: dict[str, Any],
) -> str:
    updated_result = deepcopy(previous_result)
    current_plan = updated_result.get("current_plan", {})
    sessions = sort_workout_sessions(current_plan.get("workout_sessions", []))
    if not sessions:
        return "There is no training cycle to update yet."

    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    target_session = select_cycle_target_session(
        sessions=sessions,
        current_date=current_date,
        target_date=str(arguments.get("target_date", "")).strip(),
        target_day=str(arguments.get("target_day", "")).strip(),
        target_session_hint=str(arguments.get("target_session", "")).strip(),
    )
    if not target_session:
        return "I need a target day or date in the current cycle before I can update the cycle plan."
    if same_iso_date(target_session.get("scheduled_date"), current_date):
        return "That request targets today's plan. Please ask AI Coach to adjust Today's Plan directly."

    operation = str(arguments.get("cycle_operation") or "").strip()
    if operation not in {"replace_focus", "replace_exercise", "adjust_volume", "cancel_session"}:
        operation = infer_cycle_operation_from_arguments(user_message, arguments)

    target_date = safe_iso_date(target_session.get("scheduled_date")) or str(target_session.get("scheduled_date", ""))
    if operation == "cancel_session":
        cancel_cycle_session(target_session, arguments)
        summary = f"{target_date} was cancelled."
        action_type = "cycle_cancel_session"
    elif operation == "replace_focus":
        focus = str(arguments.get("focus_category") or focus_category_from_chat_text(user_message) or "").strip()
        if focus not in ALLOWED_FOCUS_CATEGORIES or not focus:
            return "I understood that you want to change a cycle session, but I need the target training focus."
        replacement = build_cycle_focus_session(previous_result, target_session, focus)
        target_session.clear()
        target_session.update(replacement)
        summary = f"{target_date} was changed to {target_session.get('focus', focus)}."
        action_type = "cycle_replace_focus"
    elif operation == "replace_exercise":
        summary = replace_cycle_session_exercises(target_session, previous_result, user_message, arguments)
        if not summary:
            return "I could not find suitable same-focus alternatives for that cycle session."
        action_type = "cycle_replace_exercise"
    else:
        summary = adjust_cycle_session_volume(target_session, previous_result, arguments)
        if not summary:
            return "I could not apply that cycle volume change."
        action_type = "cycle_adjust_volume"

    current_plan["workout_sessions"] = sort_workout_sessions(sessions)
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=target_date or current_date,
        action_type=action_type,
        summary=f"AI Coach updated current cycle: {summary}",
        injury_areas=coerce_string_list(arguments.get("injury_areas"), []),
    )
    return f"Training cycle updated: {summary}"


def select_cycle_target_session(
    *,
    sessions: list[dict[str, Any]],
    current_date: str,
    target_date: str,
    target_day: str,
    target_session_hint: str,
) -> dict[str, Any]:
    safe_target_date = safe_iso_date(target_date)
    if safe_target_date:
        for session in sessions:
            if same_iso_date(session.get("scheduled_date"), safe_target_date):
                return session

    normalized_day = normalize_weekday(target_day)
    if normalized_day:
        for session in sessions:
            session_date = safe_iso_date(session.get("scheduled_date"))
            if session_date and session_date < current_date:
                continue
            if normalize_weekday(str(session.get("day", ""))) == normalized_day:
                return session

    if target_session_hint in {"tomorrow"}:
        tomorrow = (iso_to_date(current_date) + timedelta(days=1)).isoformat()
        for session in sessions:
            if same_iso_date(session.get("scheduled_date"), tomorrow):
                return session
    if target_session_hint in {"next_session", "current_cycle"}:
        for session in sessions:
            session_date = safe_iso_date(session.get("scheduled_date"))
            if session_date and session_date > current_date:
                return session
    return {}


def normalize_weekday(value: str) -> str:
    text = value.lower().strip()
    mapping = {
        "monday": ["monday", "mon", "周一", "星期一", "礼拜一"],
        "tuesday": ["tuesday", "tue", "周二", "星期二", "礼拜二"],
        "wednesday": ["wednesday", "wed", "周三", "星期三", "礼拜三"],
        "thursday": ["thursday", "thu", "周四", "星期四", "礼拜四"],
        "friday": ["friday", "fri", "周五", "星期五", "礼拜五"],
        "saturday": ["saturday", "sat", "周六", "星期六", "礼拜六"],
        "sunday": ["sunday", "sun", "周日", "周天", "星期日", "星期天", "礼拜日", "礼拜天"],
    }
    for canonical, aliases in mapping.items():
        if text == canonical or any(alias == text or alias in text for alias in aliases):
            return canonical
    return ""


def infer_cycle_operation_from_arguments(user_message: str, arguments: dict[str, Any]) -> str:
    if cycle_cancel_requested(user_message):
        return "cancel_session"
    if str(arguments.get("focus_category", "")).strip() or focus_category_from_chat_text(user_message):
        return "replace_focus"
    if workout_volume_change_present(arguments):
        return "adjust_volume"
    return "replace_exercise"


def cancel_cycle_session(session: dict[str, Any], arguments: dict[str, Any]) -> None:
    injury_reported = bool(arguments.get("injury_reported"))
    injury_areas = coerce_string_list(arguments.get("injury_areas"), [])
    session["is_cancelled"] = True
    session["focus"] = "Workout Cancelled"
    session["duration_minutes"] = 0
    session["warmup"] = []
    session["exercises"] = []
    session["cooldown"] = []
    session["safety_notes"] = cancel_safety_notes(injury_reported=injury_reported, injury_areas=injury_areas)
    session["injury_reported"] = injury_reported
    session["injury_areas"] = injury_areas


def build_cycle_focus_session(
    previous_result: FitnessAgentState | dict[str, Any],
    target_session: dict[str, Any],
    focus_category: str,
) -> dict[str, Any]:
    user_profile = previous_result.get("user_profile", {})
    current_plan = previous_result.get("current_plan", {})
    constraints = previous_result.get("constraints", {})
    fitness_level = str(user_profile.get("fitness_level", "beginner")).strip().lower()
    training_goal = _map_goal_tag(str(previous_result.get("goals", {}).get("primary_goal", "weight_loss")))
    equipment_access = coerce_string_list(
        constraints.get("equipment_access"),
        ["bodyweight", "dumbbell", "barbell", "bench", "rack", "cable_machine", "kettlebell", "box"],
    )
    blueprint = _build_session_blueprint(
        day=str(target_session.get("day", "")),
        scheduled_date=str(target_session.get("scheduled_date", "")),
        cycle_number=int(target_session.get("cycle_number") or current_plan.get("cycle_number") or 1),
        cycle_session_index=int(target_session.get("cycle_session_index") or 1),
        focus=focus_category,
        duration_minutes=int(target_session.get("duration_minutes") or 60),
        fitness_level=fitness_level,
        training_goal=training_goal,
        equipment_access=equipment_access,
        excluded_conditions=coerce_string_list(constraints.get("pain_sensitive_areas"), []),
        excluded_exercises=[],
    )
    replacement = dict(
        _finalize_workout_session(
            blueprint=blueprint,
            session_payload={},
            fitness_level=fitness_level,
            training_goal=training_goal,
        )
    )
    old_set_policy = set_policy_from_session(target_session)
    if old_set_policy != 4:
        apply_set_policy_to_session(replacement, old_set_policy)
    return replacement


def replace_cycle_session_exercises(
    session: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    user_message: str,
    arguments: dict[str, Any],
) -> str:
    exercises = list(session.get("exercises", []))
    if session.get("is_cancelled") or not exercises:
        return ""
    focus_tag = focus_tag_from_session(session)
    if not focus_tag:
        return ""
    target_text = str(arguments.get("target", "")).strip() or user_message
    target_index = exercise_index_from_chat_message(target_text, exercises)
    level = str(previous_result.get("user_profile", {}).get("fitness_level", "beginner"))
    current_names = [str(exercise.get("name", "")).strip() for exercise in exercises if str(exercise.get("name", "")).strip()]
    excluded_names = list(current_names)
    replacements_made: list[tuple[str, str]] = []
    target_indexes = range(len(exercises)) if target_index is None else [target_index]
    for index in target_indexes:
        original = exercises[index]
        original_name = str(original.get("name", "")).strip()
        if not original_name:
            continue
        candidates = search_similar_exercises(
            exercise_name=original_name,
            focus=focus_tag,
            level=level,
            exclude=excluded_names,
            limit=8,
        )
        candidates = same_focus_replacement_candidates(candidates, focus_tag)
        if not candidates:
            continue
        replacement = candidates[0]
        exercises[index] = replacement_exercise_payload(original, replacement)
        replacement_name = str(replacement.get("name", "")).strip()
        excluded_names.append(replacement_name)
        replacements_made.append((original_name, replacement_name))
    if not replacements_made:
        return ""
    session["exercises"] = exercises
    session.pop("is_cancelled", None)
    replacement_text = ", ".join(f"{old} -> {new}" for old, new in replacements_made)
    return f"{session.get('scheduled_date', '')} kept {session.get('focus', '')} and replaced same-focus exercises: {replacement_text}."


def adjust_cycle_session_volume(
    session: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    arguments: dict[str, Any],
) -> str:
    exercises = list(session.get("exercises", []))
    if session.get("is_cancelled") or not exercises:
        return ""
    set_adjustment = str(arguments.get("set_adjustment", "")).strip()
    exercise_count_adjustment = str(arguments.get("exercise_count_adjustment", "")).strip()
    exercise_count_delta = clamp_int(arguments.get("exercise_count_delta"), -8, 8, 0)
    intensity = str(arguments.get("intensity_adjustment", "")).strip()
    if set_adjustment:
        for exercise in exercises:
            current_sets = clamp_int(exercise.get("sets"), 3, 5, 4)
            if set_adjustment == "increase":
                exercise["sets"] = min(5, current_sets + 1)
            elif set_adjustment == "decrease":
                exercise["sets"] = max(3, current_sets - 1)
            elif set_adjustment == "target":
                exercise["sets"] = clamp_int(arguments.get("set_target"), 3, 5, current_sets)
        session["exercises"] = exercises
        return f"{session.get('scheduled_date', '')} set volume was updated."
    if exercise_count_adjustment:
        if exercise_count_adjustment == "increase":
            focus = focus_tag_from_session(session)
            additions = max(1, exercise_count_delta) if exercise_count_delta > 0 else 1
            for _ in range(additions):
                names = [
                    str(exercise.get("name", "")).strip()
                    for exercise in exercises
                    if str(exercise.get("name", "")).strip()
                ]
                candidates = search_similar_exercises(
                    exercise_name=names[0] if names else "",
                    focus=focus,
                    level=str(previous_result.get("user_profile", {}).get("fitness_level", "beginner")),
                    exclude=names,
                    limit=8,
                )
                candidates = same_focus_replacement_candidates(candidates, focus)
                if not candidates:
                    break
                exercises.append(
                    replacement_exercise_payload(
                        {"sets": set_policy_from_exercises(exercises), "reps": rep_policy_from_exercises(exercises)},
                        candidates[0],
                    )
                )
        elif exercise_count_adjustment in {"decrease", "target"}:
            default_target = max(2, len(exercises) + exercise_count_delta) if exercise_count_delta < 0 else max(2, len(exercises) - 1)
            target = clamp_int(arguments.get("exercise_count_target"), 2, 8, default_target)
            exercises = exercises[:target]
        session["exercises"] = exercises
        return f"{session.get('scheduled_date', '')} exercise count was updated."
    if intensity in {"higher", "lower"}:
        session["exercises"] = patch_exercises_for_intensity(
            exercises=exercises,
            fitness_level=str(previous_result.get("user_profile", {}).get("fitness_level", "beginner")).strip().lower(),
            intensity=intensity,
            focus=focus_tag_from_session(session),
        )
        return f"{session.get('scheduled_date', '')} intensity was adjusted."
    return ""


def execute_ai_plan_patch(
    *,
    user_message: str,
    profile_inputs: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    normalized: dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    direct_focus_summary = patch_today_focus_from_chat(
        user_message=user_message,
        previous_result=previous_result,
        normalized=normalized,
        session_state=session_state,
    )
    if direct_focus_summary:
        return direct_focus_summary

    state = build_change_request_state(
        profile_inputs=profile_inputs,
        previous_result=previous_result,
        change_request=user_message,
        normalized_change_request=normalized,
        session_state=session_state,
    )
    result = run_agent(state)
    patched_result = merge_ai_patch_result(previous_result, result, normalized)
    session_state["agent_result"] = patched_result
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=str(state.get("current_date", "")),
        action_type="ai_plan_patch",
        summary=str(normalized.get("summary") or state.get("plan_change_request") or "AI Coach updated today's plan."),
        injury_areas=coerce_string_list(normalized.get("injury_areas"), []),
    )
    today_session = select_today_session(
        sort_workout_sessions(patched_result.get("current_plan", {}).get("workout_sessions", [])),
        current_interaction_date(active_date=session_state.get("active_date"), result=patched_result),
    )
    return summarize_session_for_chat(today_session)


def _swap_duplicate_focus_session(
    sessions: list[dict[str, Any]],
    current_date: str,
    new_focus: str,
    original_focus_tag: str,
    updated_result: dict[str, Any],
) -> None:
    """After changing today to `new_focus`, find the first other session that already
    has that same focus and give it today's original focus (swap), or mark it as a
    rest day if today was ad hoc (no original plan)."""
    for i, session in enumerate(sessions):
        if same_iso_date(session.get("scheduled_date"), current_date):
            continue
        if session.get("is_cancelled") or session.get("is_rest"):
            continue
        if focus_tag_from_session(session) != new_focus:
            continue
        if original_focus_tag:
            swapped = build_cycle_focus_session(updated_result, session, original_focus_tag)
            swapped["scheduled_date"] = session["scheduled_date"]
            swapped["day"] = session.get("day", "")
            swapped["cycle_number"] = session.get("cycle_number")
            swapped["cycle_session_index"] = session.get("cycle_session_index")
            swapped["duration_minutes"] = session.get("duration_minutes")
            sessions[i] = swapped
        else:
            sessions[i] = {**session, "focus": "Rest Day", "exercises": [], "is_rest": True}
        break


def patch_today_focus_from_chat(
    *,
    user_message: str,
    previous_result: FitnessAgentState | dict[str, Any],
    normalized: dict[str, Any],
    session_state: dict[str, Any],
) -> str:
    focus = str(normalized.get("focus_category") or focus_category_from_chat_text(user_message) or "").strip()
    if focus not in ALLOWED_FOCUS_CATEGORIES or not focus:
        return ""
    if normalized.get("cancel_today") or normalized.get("injury_reported"):
        return ""
    if str(normalized.get("target_date", "")).strip() or str(normalized.get("target_day", "")).strip():
        return ""

    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    updated_result = deepcopy(previous_result)
    current_plan = updated_result.setdefault("current_plan", {})
    sessions = sort_workout_sessions(current_plan.get("workout_sessions", []))
    target_session = select_today_session(sessions, current_date)

    parsed_date = iso_to_date(current_date)
    is_ad_hoc_today = not target_session
    if not target_session:
        target_session = {
            "day": parsed_date.strftime("%A"),
            "scheduled_date": current_date,
            "cycle_number": current_plan.get("cycle_number", 1),
            "cycle_session_index": len(sessions) + 1,
            "duration_minutes": int(current_plan.get("minutes_per_session") or 60),
            "is_ad_hoc": True,
            "exercises": [],
        }
        sessions.append(target_session)

    # Capture today's original focus before overwriting (used for smart swap below)
    original_focus_tag = "" if is_ad_hoc_today else focus_tag_from_session(target_session)

    replacement = build_cycle_focus_session(updated_result, target_session, focus)
    if target_session.get("is_ad_hoc"):
        replacement["is_ad_hoc"] = True
    replacement["scheduled_date"] = current_date
    replacement["day"] = parsed_date.strftime("%A")
    replacement.pop("is_cancelled", None)

    replaced = False
    for index, session in enumerate(sessions):
        if same_iso_date(session.get("scheduled_date"), current_date):
            sessions[index] = replacement
            replaced = True
            break
    if not replaced:
        sessions.append(replacement)

    # Smart swap: if another cycle session already has the same focus we just
    # assigned to today, replace it with today's original focus so the cycle
    # stays balanced (no duplicate focus days).
    _swap_duplicate_focus_session(
        sessions=sessions,
        current_date=current_date,
        new_focus=focus,
        original_focus_tag=original_focus_tag,
        updated_result=updated_result,
    )

    current_plan["workout_sessions"] = sort_workout_sessions(sessions)
    updated_result["current_date"] = current_date
    refresh_youtube_resources(updated_result)
    session_state["agent_result"] = updated_result
    record_memory_plan_modification(
        session_state=session_state,
        date_iso=current_date,
        action_type="update_today_focus",
        summary=f"AI Coach changed today's workout focus to {replacement.get('focus', focus)}.",
        injury_areas=coerce_string_list(normalized.get("injury_areas"), []),
    )
    return summarize_session_for_chat(replacement)


def build_change_request_state(
    *,
    profile_inputs: dict[str, Any],
    previous_result: FitnessAgentState | dict[str, Any],
    change_request: str,
    normalized_change_request: dict[str, Any],
    session_state: dict[str, Any],
) -> FitnessAgentState:
    target_date = current_interaction_date(active_date=session_state.get("active_date"), result=previous_result)
    previous_feedback = dict(previous_result.get("latest_feedback", {}))
    previous_state = dict(previous_result.get("current_state", {}))
    change_request_context = build_change_request_context(change_request, normalized_change_request)
    previous_notes = " ".join(part for part in [previous_state.get("notes", ""), change_request_context] if part)
    previous_feedback["performance_notes"] = previous_notes

    base_state = build_initial_state(
        profile_inputs=profile_inputs,
        thread_id=session_state["thread_id"],
        active_date=target_date,
        memory_store=normalize_memory_store(session_state.get("memory_store")),
        session_state=session_state,
        previous_result=previous_result,
    )
    return {
        "thread_id": session_state["thread_id"],
        "current_date": target_date,
        "profile_notes": profile_inputs.get("profile_notes", ""),
        "plan_change_request": change_request,
        "normalized_change_request": normalized_change_request,
        "user_profile": base_state["user_profile"],
        "constraints": base_state["constraints"],
        "goals": base_state["goals"],
        "current_state": {**previous_state, "date": target_date, "notes": previous_notes},
        "latest_feedback": {**previous_feedback, "date": target_date},
        "current_plan": previous_result.get("current_plan", {}),
        "plan_history": previous_result.get("plan_history", []),
        "daily_history": previous_result.get("daily_history", []),
        "feedback_history": previous_result.get("feedback_history", []),
        "state_history": append_unique_history_item(
            previous_result.get("state_history", []),
            previous_result.get("current_state", {}),
            "date",
        ),
        "memory_context": base_state.get("memory_context", {}),
    }


def merge_ai_patch_result(
    previous_result: FitnessAgentState | dict[str, Any],
    result: FitnessAgentState | dict[str, Any],
    normalized: dict[str, Any],
) -> FitnessAgentState:
    patched_result: FitnessAgentState = dict(result)
    previous_plan = deepcopy(previous_result.get("current_plan", {}))
    result_plan = deepcopy(result.get("current_plan", {}))

    request_type = str(normalized.get("request_type", "")).strip()
    workout_changed = normalized_request_has_workout_change(normalized)
    nutrition_changed = bool(
        normalized.get("temporary_food_avoidances")
        or normalized.get("permanent_food_preferences")
        or request_type == "nutrition_change"
    )
    if workout_changed and not nutrition_changed and previous_plan:
        result_plan["nutrition_targets"] = deepcopy(previous_plan.get("nutrition_targets", result_plan.get("nutrition_targets", {})))
        result_plan["meal_suggestions"] = deepcopy(previous_plan.get("meal_suggestions", result_plan.get("meal_suggestions", [])))
    elif nutrition_changed and not workout_changed and previous_plan:
        result_plan["workout_sessions"] = deepcopy(previous_plan.get("workout_sessions", result_plan.get("workout_sessions", [])))

    if workout_changed and previous_plan and not str(normalized.get("set_adjustment", "")).strip():
        target_date = str(result.get("current_date") or previous_result.get("current_date") or "")
        previous_today = select_today_session(sort_workout_sessions(previous_plan.get("workout_sessions", [])), target_date)
        result_today = select_today_session(sort_workout_sessions(result_plan.get("workout_sessions", [])), target_date)
        previous_set_policy = set_policy_from_session(previous_today)
        if result_today and previous_set_policy != 4:
            apply_set_policy_to_session(result_today, previous_set_policy)

    if previous_plan:
        for key in ["summary", "objective_alignment", "coaching_focus", "recovery_actions"]:
            if key in previous_plan:
                result_plan[key] = deepcopy(previous_plan[key])

    patched_result["current_plan"] = result_plan
    patched_result["plan_history"] = deepcopy(previous_result.get("plan_history", []))
    patched_result["daily_history"] = deepcopy(previous_result.get("daily_history", []))
    patched_result["feedback_history"] = deepcopy(previous_result.get("feedback_history", []))
    patched_result["state_history"] = deepcopy(previous_result.get("state_history", []))
    refresh_youtube_resources(patched_result)
    return patched_result


def refresh_result_memory_context(result: FitnessAgentState | dict[str, Any], session_state: dict[str, Any]) -> None:
    if not isinstance(result, dict) or not result:
        return
    result["memory_context"] = memory_context_for_planning(
        session_state.get("memory_store", default_memory_store()),
        current_interaction_date(active_date=session_state.get("active_date"), result=result),
        profile_inputs=session_state.get("profile_inputs") or {},
        result=result,
        session_state=session_state,
    )


def build_chat_context(result: FitnessAgentState | dict[str, Any], session_state: dict[str, Any]) -> str:
    if not result:
        return "No plan has been generated yet."
    refresh_result_memory_context(result, session_state)
    current_date = current_interaction_date(active_date=session_state.get("active_date"), result=result)
    current_plan = result.get("current_plan", {})
    today_session = select_today_session(sort_workout_sessions(current_plan.get("workout_sessions", [])), current_date)
    exercises = [
        {
            "name": exercise.get("name", ""),
            "prescription": f"{exercise.get('sets', '')}x{exercise.get('reps', '')}".strip(),
            "primary_muscles": exercise.get("primary_muscles", []),
            "coaching_cue": exercise.get("coaching_cue", ""),
            "why_this_exercise": exercise.get("why_this_exercise", ""),
            "common_mistake": exercise.get("common_mistake", ""),
            "regression": exercise.get("regression", ""),
            "progression": exercise.get("progression", ""),
            "knowledge_source": exercise.get("knowledge_source", ""),
        }
        for exercise in today_session.get("exercises", [])
    ] if today_session else []
    latest_feedback = result.get("latest_feedback", {})
    cycle_sessions = [
        {
            "day": session.get("day", ""),
            "scheduled_date": session.get("scheduled_date", ""),
            "focus": session.get("focus", ""),
            "is_cancelled": bool(session.get("is_cancelled")),
            "exercises": [
                str(exercise.get("name", "")).strip()
                for exercise in session.get("exercises", [])
                if str(exercise.get("name", "")).strip()
            ],
        }
        for session in sort_workout_sessions(current_plan.get("workout_sessions", []))
    ]
    return json.dumps(
        {
            "active_date": current_date,
            "goal": result.get("goals", {}).get("primary_goal", ""),
            "fitness_level": result.get("user_profile", {}).get("fitness_level", ""),
            "planning_rules": {
                "baseline_beginner_exercises": 2,
                "baseline_intermediate_exercises": 3,
                "baseline_advanced_exercises": 4,
                "default_sets_per_exercise": 4,
                "explicit_set_change_bounds": "3-5 sets",
                "minimum_exercises": 2,
            },
            "cycle": {
                "number": current_plan.get("cycle_number"),
                "start": current_plan.get("cycle_start_date"),
                "end": current_plan.get("cycle_end_date"),
                "sessions": cycle_sessions,
            },
            "today_session": {
                "focus": today_session.get("focus", "") if today_session else "",
                "scheduled_date": today_session.get("scheduled_date", "") if today_session else "",
                "is_cancelled": bool(today_session.get("is_cancelled")) if today_session else False,
                "exercises": exercises,
            },
            "nutrition_targets": current_plan.get("nutrition_targets", {}),
            "latest_feedback": {
                "date": latest_feedback.get("date", ""),
                "emoji": latest_feedback.get("feeling_emoji", ""),
                "notes": latest_feedback.get("performance_notes", ""),
            },
            "memory": {
                "session_metadata": result.get("memory_context", {}).get("session_metadata", {}),
                "structured_profile": result.get("memory_context", {}).get("structured_profile", {}),
                "conversation_summary": result.get("memory_context", {}).get("conversation_summary", ""),
                "sliding_window": result.get("memory_context", {}).get("sliding_window", []),
            },
        },
        ensure_ascii=True,
    )




def normalize_change_request(change_request: str) -> dict[str, Any]:
    if not change_request.strip():
        return {}
    try:
        normalized = call_model_json(
            system_prompt=load_prompt("change_request_prompt.txt"),
            user_prompt=change_request,
            temperature=0.0,
            max_tokens=600,
        )
    except Exception:
        normalized = {}
    return sanitize_normalized_change_request(normalized)


def sanitize_normalized_change_request(normalized: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(normalized, dict):
        normalized = {}
    request_type = str(normalized.get("request_type", "unclear")).strip()
    scope = str(normalized.get("scope", "unclear")).strip()
    focus_category = str(normalized.get("focus_category", "")).strip()
    intent = str(normalized.get("intent", "unclear")).strip()
    return {
        "intent": intent if intent in ALLOWED_NORMALIZED_INTENTS else "unclear",
        "request_type": request_type if request_type in ALLOWED_CHANGE_REQUEST_TYPES else "unclear",
        "scope": scope if scope in ALLOWED_CHANGE_REQUEST_SCOPES else "unclear",
        "focus_category": focus_category if focus_category in ALLOWED_FOCUS_CATEGORIES else "",
        "keep_current_focus": bool(normalized.get("keep_current_focus", False)),
        "target": str(normalized.get("target", "")).strip(),
        "injury_reported": bool(normalized.get("injury_reported", False)),
        "injury_areas": coerce_string_list(normalized.get("injury_areas"), []),
        "cancel_today": bool(normalized.get("cancel_today", False)),
        "recovery_signal_only": bool(normalized.get("recovery_signal_only", False)),
        "intensity_adjustment": str(normalized.get("intensity_adjustment", "")).strip(),
        "set_adjustment": str(normalized.get("set_adjustment", "")).strip(),
        "set_target": clamp_int(normalized.get("set_target"), 0, 12, 0),
        "exercise_count_adjustment": str(normalized.get("exercise_count_adjustment", "")).strip(),
        "exercise_count_target": clamp_int(normalized.get("exercise_count_target"), 0, 8, 0),
        "duration_adjustment": str(normalized.get("duration_adjustment", "")).strip(),
        "target_date": str(normalized.get("target_date", "")).strip(),
        "target_day": str(normalized.get("target_day", "")).strip(),
        "target_session": str(normalized.get("target_session", "")).strip(),
        "cycle_operation": str(normalized.get("cycle_operation") or normalized.get("operation") or "").strip(),
        "temporary_food_avoidances": coerce_string_list(normalized.get("temporary_food_avoidances"), []),
        "permanent_food_preferences": coerce_string_list(normalized.get("permanent_food_preferences"), []),
        "summary": str(normalized.get("summary", "")).strip(),
        "confidence": clamp_float(normalized.get("confidence"), 0.0, 1.0, 0.0),
    }


def infer_missing_intent_from_normalized(normalized: dict[str, Any]) -> dict[str, Any]:
    inferred = dict(normalized)
    intent = str(inferred.get("intent", "")).strip()
    if intent and intent not in {"unclear", "answer_question"}:
        return inferred
    if inferred.get("injury_reported") or inferred.get("cancel_today"):
        inferred["intent"] = "cancel_workout"
    elif coerce_string_list(inferred.get("temporary_food_avoidances"), []):
        inferred["intent"] = "replace_food"
    elif str(inferred.get("set_adjustment", "")).strip():
        inferred["intent"] = "adjust_workout_volume"
    elif str(inferred.get("exercise_count_adjustment", "")).strip():
        inferred["intent"] = "adjust_workout_volume"
    elif str(inferred.get("intensity_adjustment", "")).strip():
        inferred["intent"] = "adjust_workout_volume"
    elif chat_text_requests_add_exercise(str(inferred.get("summary", ""))):
        inferred["intent"] = "adjust_workout_volume"
    elif str(inferred.get("focus_category", "")).strip():
        inferred["intent"] = "update_today_plan"
    else:
        inferred["intent"] = "unclear"
    return inferred




def patch_exercises_for_intensity(
    *,
    exercises: list[dict[str, Any]],
    fitness_level: str,
    intensity: str,
    focus: str,
) -> list[dict[str, Any]]:
    patched = [dict(exercise) for exercise in exercises]
    current_set_policy = set_policy_from_exercises(patched)
    if intensity == "lower":
        target_count = 2 if fitness_level in {"beginner", "intermediate"} else 3
        reps = "6-8" if fitness_level == "beginner" else "10-12"
        note = "Lower intensity: keep the load conservative, move with control, and stop well before form breaks."
    else:
        reps = "8-10" if fitness_level == "beginner" else "12-15"
        note = "Higher intensity: use controlled tempo, full range of motion, and a brief pause while keeping clean form."

    if intensity == "lower":
        patched = patched[: max(2, min(target_count, len(patched)))]
    elif patched:
        current_names = [str(exercise.get("name", "")).strip() for exercise in patched if str(exercise.get("name", "")).strip()]
        seed_name = current_names[0] if current_names else ""
        additions = search_similar_exercises(
            exercise_name=seed_name,
            focus=focus,
            level=fitness_level,
            exclude=current_names,
            limit=3,
        ) if seed_name else []
        if additions:
            patched.append(replacement_exercise_payload({"sets": current_set_policy, "reps": reps}, additions[0]))
    for exercise in patched:
        exercise["sets"] = current_set_policy
        exercise["reps"] = reps
        existing_note = str(exercise.get("notes", "")).strip()
        if note not in existing_note:
            exercise["notes"] = f"{existing_note} {note}".strip()
    return patched


def protect_future_sessions_for_injury(
    *,
    updated_result: FitnessAgentState | dict[str, Any],
    current_date: str,
    injury_areas: list[str],
) -> list[str]:
    current_plan = updated_result.get("current_plan", {})
    sessions = sort_workout_sessions(current_plan.get("workout_sessions", []))
    current_cycle = cycle_number_for_date(current_plan, current_date)
    protected_labels: list[str] = []
    for session in sessions:
        session_date = safe_iso_date(session.get("scheduled_date"))
        if not session_date or session_date <= current_date:
            continue
        if session.get("cycle_number", current_cycle) != current_cycle:
            continue
        if session.get("is_cancelled") or not session_stresses_injury_area(session, injury_areas):
            continue
        session["is_cancelled"] = True
        session["focus"] = "Recovery (injury protection)"
        session["duration_minutes"] = 0
        session["warmup"] = []
        session["exercises"] = []
        session["cooldown"] = []
        session["injury_reported"] = True
        session["injury_areas"] = injury_areas
        session["safety_notes"] = [
            f"Protected because a recent injury was reported around {', '.join(injury_areas)}.",
            "Resume this focus only after later feedback indicates symptoms have improved.",
        ]
        protected_labels.append(f"{session_date} {session.get('day', '')}".strip())
    return protected_labels


def cycle_number_for_date(current_plan: dict[str, Any], date_iso: str) -> int:
    for session in current_plan.get("workout_sessions", []):
        if same_iso_date(session.get("scheduled_date"), date_iso):
            return int(session.get("cycle_number") or current_plan.get("cycle_number") or 1)
    return int(current_plan.get("cycle_number") or 1)


def session_stresses_injury_area(session: dict[str, Any], injury_areas: list[str]) -> bool:
    session_text = " ".join(
        [
            str(session.get("focus", "")),
            " ".join(str(exercise.get("name", "")) for exercise in session.get("exercises", [])),
            " ".join(str(exercise.get("target_muscle", "")) for exercise in session.get("exercises", [])),
        ]
    ).lower()
    stress_aliases = {
        "back": ["back", "lat", "row", "pulldown", "deadlift", "hinge", "squat"],
        "knee": ["lower body", "leg", "squat", "lunge", "knee", "glute"],
        "shoulder": ["shoulder", "press", "chest", "push", "row"],
        "ankle": ["lower body", "leg", "lunge", "squat", "jump", "conditioning"],
        "hip": ["lower body", "leg", "glute", "squat", "lunge", "hinge"],
        "wrist": ["push", "press", "curl", "row", "plank"],
        "elbow": ["push", "press", "curl", "row"],
    }
    for area in injury_areas or ["reported injury area"]:
        normalized_area = str(area).lower()
        aliases = stress_aliases.get(normalized_area, [normalized_area])
        if any(alias in session_text for alias in aliases):
            return True
    return False


def cancel_safety_notes(*, injury_reported: bool, injury_areas: list[str]) -> list[str]:
    if injury_reported:
        area_text = f" around {', '.join(injury_areas)}" if injury_areas else ""
        return [
            f"Today's workout was cancelled because an injury was reported{area_text}.",
            "Do not train through injury symptoms; consider medical guidance before resuming.",
        ]
    return ["Today's workout was cancelled at your request."]


def augment_chat_safety_change(user_message: str, normalized: dict[str, Any]) -> dict[str, Any]:
    injury_areas = injury_areas_from_chat_text(user_message)
    cancel_requested = chat_text_requests_cancel(user_message)
    mild_recovery_signal = bool(injury_areas) and not cancel_requested and not chat_text_has_hard_injury_signal(user_message)
    if normalized.get("injury_reported") or normalized.get("cancel_today"):
        if mild_recovery_signal:
            augmented = dict(normalized)
            augmented["request_type"] = "recovery_change"
            augmented["injury_reported"] = True
            augmented["injury_areas"] = coerce_string_list(augmented.get("injury_areas"), []) or injury_areas
            augmented["cancel_today"] = False
            augmented["recovery_signal_only"] = True
            summary = "User reported body-area discomfort; protect related future cycle sessions and only cancel today if today's session stresses that area."
            augmented["summary"] = " ".join(part for part in [str(augmented.get("summary", "")).strip(), summary] if part)
            return augmented
        return normalized
    if not injury_areas and not cancel_requested:
        return normalized
    augmented = dict(normalized)
    if cycle_change_target_present(augmented):
        augmented["request_type"] = "recovery_change" if injury_areas else "workout_change"
        augmented["scope"] = "current_cycle"
        augmented["intent"] = "update_cycle_plan"
        augmented["cycle_operation"] = "cancel_session"
        augmented["cancel_today"] = False
        augmented["injury_reported"] = bool(injury_areas)
        augmented["injury_areas"] = injury_areas
        summary = "User requested cancelling a targeted cycle session."
        augmented["summary"] = " ".join(part for part in [str(augmented.get("summary", "")).strip(), summary] if part)
        augmented["confidence"] = max(float(augmented.get("confidence") or 0.0), 0.88)
        return augmented
    augmented["request_type"] = "recovery_change" if injury_areas else "workout_change"
    augmented["scope"] = "today_only"
    recovery_signal_only = mild_recovery_signal
    augmented["cancel_today"] = not recovery_signal_only
    augmented["recovery_signal_only"] = recovery_signal_only
    augmented["injury_reported"] = bool(injury_areas)
    augmented["injury_areas"] = injury_areas
    if injury_areas and recovery_signal_only:
        summary = "User reported body-area discomfort; protect related future cycle sessions and only cancel today if today's session stresses that area."
    elif injury_areas:
        summary = "User reported injury; cancel today's workout."
    else:
        summary = "User requested cancelling today's workout without an injury reason."
    augmented["summary"] = " ".join(part for part in [str(augmented.get("summary", "")).strip(), summary] if part)
    augmented["confidence"] = max(float(augmented.get("confidence") or 0.0), 0.85)
    return augmented


def augment_chat_food_change(user_message: str, normalized: dict[str, Any], result: FitnessAgentState | dict[str, Any]) -> dict[str, Any]:
    if coerce_string_list(normalized.get("temporary_food_avoidances"), []):
        return normalized
    avoidances = food_avoidances_from_chat_text(user_message, result)
    if not avoidances:
        return normalized
    augmented = dict(normalized)
    augmented["request_type"] = "nutrition_change"
    augmented["scope"] = "today_only"
    augmented["temporary_food_avoidances"] = avoidances
    summary = f"User wants to avoid {', '.join(avoidances)} today."
    augmented["summary"] = " ".join(part for part in [str(augmented.get("summary", "")).strip(), summary] if part)
    augmented["confidence"] = max(float(augmented.get("confidence") or 0.0), 0.85)
    return augmented


def augment_chat_focus_change(user_message: str, normalized: dict[str, Any]) -> dict[str, Any]:
    if normalized.get("focus_category") or normalized.get("injury_reported") or normalized.get("cancel_today"):
        return normalized
    focus_category = focus_category_from_chat_text(user_message)
    if not focus_category:
        return normalized
    augmented = dict(normalized)
    augmented["request_type"] = "workout_change"
    augmented["scope"] = "today_only"
    augmented["focus_category"] = focus_category
    summary = f"User asked to train focus category={focus_category} today."
    augmented["summary"] = " ".join(part for part in [str(augmented.get("summary", "")).strip(), summary] if part)
    augmented["confidence"] = max(float(augmented.get("confidence") or 0.0), 0.85)
    return augmented


def augment_chat_cycle_change(user_message: str, normalized: dict[str, Any], result: FitnessAgentState | dict[str, Any]) -> dict[str, Any]:
    if str(normalized.get("scope", "")) == "today_only" and "today" in user_message.lower():
        return normalized
    target_date, target_day, target_session_hint = cycle_target_from_text(user_message, result)
    explicit_cycle_scope = str(normalized.get("scope", "")) in {"current_cycle", "future_default"}
    if not target_date and not target_day and not target_session_hint and not explicit_cycle_scope:
        return normalized

    augmented = dict(normalized)
    augmented["intent"] = "update_cycle_plan"
    augmented["request_type"] = "workout_change"
    augmented["scope"] = "current_cycle"
    if target_date:
        augmented["target_date"] = target_date
    if target_day:
        augmented["target_day"] = target_day
    if target_session_hint:
        augmented["target_session"] = target_session_hint

    text = user_message.lower()
    if normalized.get("injury_reported") or injury_areas_from_chat_text(user_message):
        augmented["cycle_operation"] = "cancel_session"
        augmented["injury_reported"] = True
        augmented["injury_areas"] = coerce_string_list(augmented.get("injury_areas"), []) or injury_areas_from_chat_text(user_message)
    elif cycle_cancel_requested(text):
        augmented["cycle_operation"] = "cancel_session"
    elif str(augmented.get("focus_category", "")).strip() or focus_category_from_chat_text(user_message):
        augmented["focus_category"] = str(augmented.get("focus_category") or focus_category_from_chat_text(user_message))
        augmented["cycle_operation"] = "replace_focus"
    elif chat_message_requests_exercise_replacement(user_message) or chat_requests_general_exercise_replacement(user_message):
        augmented["cycle_operation"] = "replace_exercise"
    elif workout_volume_change_present(augmented):
        augmented["cycle_operation"] = "adjust_volume"
    else:
        augmented["cycle_operation"] = str(augmented.get("cycle_operation") or "replace_exercise")
    summary = "User requested a current-cycle plan change."
    augmented["summary"] = " ".join(part for part in [str(augmented.get("summary", "")).strip(), summary] if part)
    augmented["confidence"] = max(float(augmented.get("confidence") or 0.0), 0.86)
    return augmented


def augment_chat_workout_volume_change(user_message: str, normalized: dict[str, Any]) -> dict[str, Any]:
    if normalized.get("injury_reported") or normalized.get("cancel_today"):
        return normalized
    augmented = dict(normalized)
    changed = False

    if not str(augmented.get("set_adjustment", "")).strip():
        set_adjustment, set_target = set_adjustment_from_text(user_message)
        if set_adjustment:
            augmented["set_adjustment"] = set_adjustment
            augmented["set_target"] = set_target
            changed = True

    if not str(augmented.get("exercise_count_adjustment", "")).strip():
        exercise_count_adjustment, exercise_count_target = exercise_count_adjustment_from_text(user_message)
        if exercise_count_adjustment:
            augmented["exercise_count_adjustment"] = exercise_count_adjustment
            augmented["exercise_count_target"] = exercise_count_target
            changed = True

    if not str(augmented.get("intensity_adjustment", "")).strip() and not str(augmented.get("set_adjustment", "")).strip():
        fallback_intensity = chat_intensity_from_text(user_message)
        if fallback_intensity:
            augmented["intensity_adjustment"] = fallback_intensity
            changed = True

    if not changed and not workout_volume_change_present(augmented):
        return normalized

    request_type = str(augmented.get("request_type", ""))
    augmented["request_type"] = (
        request_type
        if request_type in ALLOWED_CHANGE_REQUEST_TYPES and request_type not in {"none", "unclear", ""}
        else "workout_change"
    )
    scope = str(augmented.get("scope", ""))
    augmented["scope"] = scope if scope in ALLOWED_CHANGE_REQUEST_SCOPES and scope not in {"unclear", ""} else "today_only"
    augmented["intent"] = "adjust_workout_volume"
    summary = str(augmented.get("summary", "")).strip()
    volume_summary = workout_volume_summary(augmented)
    augmented["summary"] = " ".join(part for part in [summary, volume_summary] if part)
    augmented["confidence"] = max(float(augmented.get("confidence") or 0.0), 0.88)
    return augmented


def workout_volume_change_present(normalized: dict[str, Any]) -> bool:
    return bool(
        str(normalized.get("set_adjustment", "")).strip()
        or str(normalized.get("exercise_count_adjustment", "")).strip()
        or str(normalized.get("intensity_adjustment", "")).strip()
        or str(normalized.get("intent", "")).strip() in {"adjust_workout_volume", "adjust_sets", "adjust_intensity", "add_exercise", "remove_exercise"}
    )


def workout_volume_summary(normalized: dict[str, Any]) -> str:
    parts: list[str] = []
    set_adjustment = str(normalized.get("set_adjustment", "")).strip()
    if set_adjustment:
        if set_adjustment == "target" and normalized.get("set_target"):
            parts.append(f"set target {normalized['set_target']}")
        else:
            parts.append(f"{set_adjustment} sets")
    exercise_count_adjustment = str(normalized.get("exercise_count_adjustment", "")).strip()
    if exercise_count_adjustment:
        if exercise_count_adjustment == "target" and normalized.get("exercise_count_target"):
            parts.append(f"exercise count target {normalized['exercise_count_target']}")
        else:
            parts.append(f"{exercise_count_adjustment} exercise count")
    intensity = str(normalized.get("intensity_adjustment", "")).strip()
    if intensity:
        parts.append(f"{intensity} intensity")
    return "User requested workout volume change: " + ", ".join(parts) + "." if parts else ""


def chat_text_requests_cancel(text: str) -> bool:
    normalized = text.lower()
    cancel_terms = ["cancel today's plan", "cancel today", "cancel workout", "skip today", "no workout today", "取消今天", "取消训练", "今天不练"]
    return any(term in normalized for term in cancel_terms)


def cycle_cancel_requested(text: str) -> bool:
    normalized = text.lower()
    cancel_terms = ["cancel", "skip", "remove workout", "no workout", "休息", "取消", "不练", "跳过"]
    return any(term in normalized for term in cancel_terms)


def cycle_change_target_present(normalized: dict[str, Any]) -> bool:
    return bool(
        str(normalized.get("target_date", "")).strip()
        or str(normalized.get("target_day", "")).strip()
        or str(normalized.get("target_session", "")).strip()
        or str(normalized.get("scope", "")) in {"current_cycle", "future_default"}
    ) and str(normalized.get("scope", "")) != "today_only"


def cycle_target_from_text(user_message: str, result: FitnessAgentState | dict[str, Any]) -> tuple[str, str, str]:
    text = user_message.lower()
    current_date = current_interaction_date(active_date=result.get("current_date"), result=result)
    iso_match = re.search(r"\b(20\d{2}[-/]\d{1,2}[-/]\d{1,2})\b", text)
    if iso_match:
        parsed = safe_iso_date(iso_match.group(1).replace("/", "-"))
        if parsed:
            return parsed, "", ""
    md_match = re.search(r"\b(\d{1,2})[./-](\d{1,2})\b", text)
    if md_match:
        current = iso_to_date(current_date)
        parsed = safe_iso_date(f"{current.year}-{int(md_match.group(1)):02d}-{int(md_match.group(2)):02d}")
        if parsed:
            return parsed, "", ""

    if any(term in text for term in ["tomorrow", "明天"]):
        tomorrow = iso_to_date(current_date) + timedelta(days=1)
        return tomorrow.isoformat(), "", "tomorrow"
    if any(term in text for term in ["next session", "next workout", "下次训练", "下一次训练", "下一节"]):
        return "", "", "next_session"

    day_aliases = {
        "Monday": ["monday", "mon", "周一", "星期一", "礼拜一"],
        "Tuesday": ["tuesday", "tue", "周二", "星期二", "礼拜二"],
        "Wednesday": ["wednesday", "wed", "周三", "星期三", "礼拜三"],
        "Thursday": ["thursday", "thu", "周四", "星期四", "礼拜四"],
        "Friday": ["friday", "fri", "周五", "星期五", "礼拜五"],
        "Saturday": ["saturday", "sat", "周六", "星期六", "礼拜六"],
        "Sunday": ["sunday", "sun", "周日", "周天", "星期日", "星期天", "礼拜日", "礼拜天"],
    }
    for canonical, aliases in day_aliases.items():
        if any(alias in text for alias in aliases):
            return "", canonical, ""
    if any(term in text for term in ["week plan", "weekly plan", "cycle plan", "current cycle", "this cycle", "周计划", "周期计划", "这一周", "本周"]):
        return "", "", "current_cycle"
    return "", "", ""


def injury_areas_from_chat_text(text: str) -> list[str]:
    normalized = text.lower()
    area_map = {
        "back": ["back", "lower back", "背", "腰"],
        "knee": ["knee", "knees", "膝盖"],
        "shoulder": ["shoulder", "shoulders", "肩"],
        "ankle": ["ankle", "ankles", "脚踝"],
        "wrist": ["wrist", "wrists", "手腕"],
        "elbow": ["elbow", "elbows", "肘"],
        "hip": ["hip", "hips", "髋"],
    }
    areas = [area for area, aliases in area_map.items() if any(alias in normalized for alias in aliases)]
    hard_injury = chat_text_has_hard_injury_signal(text)
    mild_area_discomfort = bool(areas) and chat_text_has_mild_discomfort_signal(text)
    if not hard_injury and not mild_area_discomfort:
        return []
    return areas or ["reported injury area"]


def chat_text_has_hard_injury_signal(text: str) -> bool:
    normalized = text.lower()
    injury_terms = ["injured", "injury", "hurt", "pain", "strain", "sprain", "受伤", "疼", "痛"]
    return any(term in normalized for term in injury_terms)


def chat_text_has_mild_discomfort_signal(text: str) -> bool:
    normalized = text.lower()
    mild_terms = [
        "uncomfortable",
        "discomfort",
        "sore",
        "tight",
        "ache",
        "not feeling right",
        "不舒服",
        "不适",
        "酸",
        "紧",
        "别扭",
    ]
    return any(term in normalized for term in mild_terms)


def food_avoidances_from_chat_text(user_message: str, result: FitnessAgentState | dict[str, Any]) -> list[str]:
    text = user_message.lower()
    loose_text = normalize_loose(user_message)
    food_change_terms = ["don't want", "dont want", "do not want", "not want", "avoid", "no ", "replace", "swap", "instead of", "不想吃", "不要吃", "不吃", "换掉", "替换"]
    if not any(term in text for term in food_change_terms):
        return []
    current_food_names = [
        str(meal.get("food_name", "")).strip()
        for meal in result.get("current_plan", {}).get("meal_suggestions", [])
        if str(meal.get("food_name", "")).strip()
    ]
    matched: list[str] = []
    for food_name in current_food_names:
        loose_food = normalize_loose(food_name)
        first_word = normalize_loose(food_name.split(",", 1)[0].split("(", 1)[0])
        if loose_food and loose_food in loose_text:
            matched.append(food_name)
        elif first_word and first_word in loose_text:
            matched.append(food_name)
    if matched:
        return dedupe_preserve_order(matched)
    known_food_terms = ["broccoli", "salmon", "chicken", "rice", "yogurt", "egg", "beef", "tofu", "oats", "banana"]
    return [food for food in known_food_terms if food in text]


def focus_category_from_chat_text(user_message: str) -> str:
    text = user_message.lower()
    if injury_areas_from_chat_text(user_message):
        return ""
    action_terms = ["do", "train", "workout", "add", "switch", "change", "focus", "i want", "can i", "could i", "练", "训练", "加练", "换成", "改成"]
    if not any(term in text for term in action_terms):
        return ""
    focus_aliases = [
        ("upper_shoulders", ["shoulder", "shoulders", "delts", "肩"]),
        ("back_training", ["back training", "train back", "work back", "lats", "row", "背"]),
        ("lower_legs_glutes", ["lower body", "legs", "leg", "glutes", "glute", "squat", "腿", "臀"]),
        ("functional_core", ["core", "abs", "腹", "核心"]),
        ("functional_power", ["power", "explosive", "爆发"]),
        ("functional_conditioning", ["conditioning", "cardio", "functional", "体能", "功能性", "调节"]),
        ("upper_chest_arms", ["chest", "arms", "biceps", "triceps", "push", "胸", "手臂", "二头", "三头"]),
    ]
    for category, aliases in focus_aliases:
        if any(alias in text for alias in aliases):
            return category
    return ""


def chat_intensity_from_text(text: str) -> str:
    normalized = text.lower()
    set_adjustment, _ = set_adjustment_from_text(text)
    if set_adjustment:
        return ""
    lower_terms = ["tired", "uncomfortable", "not feeling good", "not good", "low energy", "poor sleep", "bad sleep", "didn't sleep", "dont sleep", "don't sleep", "slept badly", "sleep badly", "exhausted", "fatigued", "make it easier", "easier", "lighter", "less", "reduce", "不舒服", "累", "状态不好", "轻一点", "减量"]
    higher_terms = ["energized", "excited", "feel good", "feeling good", "strong", "do more", "more", "harder", "challenge", "too easy", "increase", "加量", "加强", "状态很好", "很兴奋", "多一点"]
    if any(term in normalized for term in lower_terms):
        return "lower"
    if any(term in normalized for term in higher_terms):
        return "higher"
    return ""


def set_adjustment_from_text(text: str) -> tuple[str, int]:
    normalized = text.lower()
    if not any(term in normalized for term in ["set", "sets", "组"]):
        return "", 0

    target_match = re.search(r"\b(?:to|do|make it|make them)?\s*([2-9])\s*sets?\b", normalized)
    if target_match:
        return "target", int(target_match.group(1))
    chinese_target_match = re.search(r"([二三四五六七八九23456789])\s*组", normalized)
    if chinese_target_match and any(term in normalized for term in ["改", "做", "变", "到", "成"]):
        target = {"二": 2, "三": 3, "四": 4, "五": 5, "六": 6, "七": 7, "八": 8, "九": 9}.get(chinese_target_match.group(1), 0) or int(chinese_target_match.group(1))
        return "target", target

    increase_terms = [
        "add set",
        "add sets",
        "more set",
        "more sets",
        "extra set",
        "extra sets",
        "one more set",
        "increase sets",
        "increase set",
        "加组",
        "多一组",
        "多几组",
        "增加组",
    ]
    decrease_terms = [
        "reduce set",
        "reduce sets",
        "fewer set",
        "fewer sets",
        "less set",
        "less sets",
        "one less set",
        "decrease sets",
        "decrease set",
        "少一组",
        "少几组",
        "减少组",
        "减组",
    ]
    if any(term in normalized for term in decrease_terms):
        return "decrease", 0
    if any(term in normalized for term in increase_terms):
        return "increase", 0
    return "", 0


def exercise_count_adjustment_from_text(text: str) -> tuple[str, int]:
    normalized = text.lower()
    if any(term in normalized for term in ["set", "sets", "组"]):
        return "", 0
    food_terms = ["food", "meal", "nutrition", "breakfast", "lunch", "dinner", "snack", "食物", "饮食", "餐"]
    if any(term in normalized for term in food_terms):
        return "", 0

    has_exercise_language = any(
        term in normalized
        for term in ["exercise", "exercises", "movement", "movements", "action", "actions", "workout item", "动作"]
    ) or any(fuzzy_contains_word(normalized, canonical) for canonical in ["exercise", "exercises", "movement", "movements"])
    if not has_exercise_language:
        return "", 0

    target_match = re.search(r"\b(?:to|do|make it|make them)?\s*([2-8])\s*(?:exercises?|movements?|actions?)\b", normalized)
    if target_match:
        return "target", int(target_match.group(1))

    decrease_terms = [
        "remove",
        "drop",
        "delete",
        "less",
        "fewer",
        "reduce",
        "one less",
        "take away",
        "减",
        "少",
        "删",
        "去掉",
    ]
    increase_terms = [
        "add",
        "one more",
        "another",
        "extra",
        "more",
        "increase",
        "再加",
        "加一个",
        "多一个",
        "多点",
        "多些",
    ]
    if any(term in normalized for term in decrease_terms):
        return "decrease", 0
    if any(term in normalized for term in increase_terms):
        return "increase", 0
    return "", 0


def chat_text_requests_add_exercise(text: str) -> bool:
    adjustment, _ = exercise_count_adjustment_from_text(text)
    return adjustment == "increase"


def chat_request_should_update_today(user_message: str, normalized: dict[str, Any]) -> bool:
    text = user_message.lower()
    update_terms = ["update", "change", "switch", "add", "make today", "today", "can i", "could i", "i want", "want", "do more", "energized", "excited", "strong", "easier", "lighter", "harder", "injury", "injured", "hurt", "pain", "sleep", "slept", "didn't sleep", "dont sleep", "don't sleep", "poor sleep", "let's", "练", "改", "换", "加", "今天"]
    has_update_language = any(term in text for term in update_terms)
    has_structured_change = bool(
        normalized.get("focus_category")
        or normalized.get("cancel_today")
        or normalized.get("set_adjustment")
        or normalized.get("exercise_count_adjustment")
        or normalized.get("intensity_adjustment")
        or normalized.get("temporary_food_avoidances")
        or normalized.get("permanent_food_preferences")
        or str(normalized.get("intent", ""))
        in {
            "update_today_plan",
            "replace_exercise",
            "replace_food",
            "adjust_workout_volume",
            "adjust_sets",
            "adjust_intensity",
            "add_exercise",
            "remove_exercise",
            "cancel_workout",
        }
    )
    request_type = str(normalized.get("request_type", ""))
    if normalized.get("injury_reported") or normalized.get("cancel_today"):
        return has_structured_change
    return has_update_language and has_structured_change and request_type in {"workout_change", "nutrition_change", "mixed_change", "recovery_change"}


def chat_message_is_advice_only_question(user_message: str, normalized: dict[str, Any]) -> bool:
    if normalized.get("injury_reported") or normalized.get("cancel_today"):
        return False
    if workout_volume_change_present(normalized):
        return False
    if coerce_string_list(normalized.get("temporary_food_avoidances"), []):
        return False
    text = user_message.lower()
    advice_terms = [
        "do you recommend",
        "would you recommend",
        "should i",
        "is it ok",
        "is it okay",
        "what do you think",
        "你建议",
        "应该",
        "可以吗",
    ]
    explicit_change_terms = [
        "update",
        "change my plan",
        "change today's plan",
        "add to my plan",
        "set today's plan",
        "make today's plan",
        "改计划",
        "更新计划",
        "加入计划",
    ]
    return any(term in text for term in advice_terms) and not any(term in text for term in explicit_change_terms)


def chat_message_requests_exercise_replacement(user_message: str) -> bool:
    text = user_message.lower()
    return chat_message_has_replacement_language(user_message) and (
        chat_requests_general_exercise_replacement(user_message)
        or any(term in text for term in ["exercise", "action", "movement", "动作"])
    )


def chat_message_has_replacement_language(user_message: str) -> bool:
    text = user_message.lower()
    replacement_terms = ["replace", "swap", "change", "alternative", "instead", "换", "替换", "不要", "不想做", "换掉", "改掉"]
    return any(term in text for term in replacement_terms)


def chat_requests_general_exercise_replacement(user_message: str) -> bool:
    text = user_message.lower()
    exercise_terms = ["exercise", "exercises", "action", "actions", "movement", "movements", "动作"]
    general_terms = ["some", "all", "another", "alternative", "alternatives", "different", "new", "几个", "一些", "全部", "换一换", "换几个", "换掉"]
    food_terms = ["food", "meal", "nutrition", "diet", "breakfast", "lunch", "dinner", "snack", "食物", "饮食", "餐"]
    has_exercise_language = any(term in text for term in exercise_terms) or any(
        fuzzy_contains_word(text, canonical) for canonical in ["exercise", "exercises", "movement", "movements"]
    )
    has_general_language = any(term in text for term in general_terms)
    has_replacement_language = chat_message_has_replacement_language(user_message)
    has_food_language = any(term in text for term in food_terms)
    if has_exercise_language and has_general_language:
        return True
    return has_replacement_language and has_general_language and not has_food_language


def _has_exercise_typo_token(text: str) -> bool:
    return any(fuzzy_word_match(token, "exercise") or fuzzy_word_match(token, "exercises") for token in re.findall(r"[a-z]+", text.lower()))


def fuzzy_contains_word(text: str, canonical: str, threshold: float = 0.84) -> bool:
    return any(fuzzy_word_match(token, canonical, threshold) for token in re.findall(r"[a-z]+", text.lower()))


def fuzzy_word_match(token: str, canonical: str, threshold: float = 0.84) -> bool:
    if len(token) < 5:
        return False
    if token == canonical:
        return True
    return SequenceMatcher(None, token, canonical).ratio() >= threshold


def exercise_index_from_chat_message(user_message: str, exercises: list[dict[str, Any]]) -> int | None:
    text = user_message.lower()
    normalized_text = normalize_loose(text)
    for index, exercise in enumerate(exercises):
        name = str(exercise.get("name", "")).strip()
        if name and normalize_loose(name) in normalized_text:
            return index
    ordinal_terms = [
        (0, ["first exercise", "first movement", "1st exercise", "第一个动作", "第1个动作", "第一个", "第1个"]),
        (1, ["second exercise", "second movement", "2nd exercise", "第二个动作", "第2个动作", "第二个", "第2个"]),
        (2, ["third exercise", "third movement", "3rd exercise", "第三个动作", "第3个动作", "第三个", "第3个"]),
        (3, ["fourth exercise", "fourth movement", "4th exercise", "第四个动作", "第4个动作", "第四个", "第4个"]),
        (4, ["fifth exercise", "fifth movement", "5th exercise", "第五个动作", "第5个动作", "第五个"]),
    ]
    for index, terms in ordinal_terms:
        if index < len(exercises) and any(term in text for term in terms):
            return index
    return None


def replacement_exercise_payload(original_exercise: dict[str, Any], replacement: dict[str, Any]) -> dict[str, Any]:
    return build_exercise_plan_payload(
        replacement,
        sets=clamp_int(original_exercise.get("sets"), 3, 5, 4),
        reps=str(original_exercise.get("reps", "")),
        notes=str(replacement.get("notes", "")),
    )


def same_focus_replacement_candidates(candidates: list[dict[str, Any]], focus_tag: str) -> list[dict[str, Any]]:
    if not focus_tag:
        return []
    focus_key = normalize_loose(focus_tag)
    filtered = []
    for candidate in candidates:
        tags = [normalize_loose(tag) for tag in coerce_string_list(candidate.get("focus_tags"), [])]
        if focus_key in tags:
            filtered.append(candidate)
    return filtered


def set_policy_from_session(session: dict[str, Any]) -> int:
    return set_policy_from_exercises(session.get("exercises", []) if session else [])


def set_policy_from_exercises(exercises: list[dict[str, Any]]) -> int:
    set_values = [
        clamp_int(exercise.get("sets"), 3, 5, 4)
        for exercise in exercises
        if isinstance(exercise, dict)
    ]
    if not set_values:
        return 4
    counts = {sets: set_values.count(sets) for sets in set_values}
    return max(counts, key=lambda sets: (counts[sets], sets))


def rep_policy_from_exercises(exercises: list[dict[str, Any]]) -> str:
    rep_values = [
        str(exercise.get("reps", "")).strip()
        for exercise in exercises
        if isinstance(exercise, dict) and str(exercise.get("reps", "")).strip()
    ]
    if not rep_values:
        return "6-10"
    counts = {reps: rep_values.count(reps) for reps in rep_values}
    return max(counts, key=lambda reps: (counts[reps], rep_values.index(reps) * -1))


def apply_set_policy_to_session(session: dict[str, Any], set_policy: int) -> None:
    bounded_policy = clamp_int(set_policy, 3, 5, 4)
    for exercise in session.get("exercises", []):
        if isinstance(exercise, dict):
            exercise["sets"] = bounded_policy


def focus_tag_from_session(session: dict[str, Any]) -> str:
    focus = str(session.get("focus", "")).lower()
    if "shoulder" in focus:
        return "upper_shoulders"
    if "back" in focus:
        return "back_training"
    if "lower" in focus or "legs" in focus or "glutes" in focus:
        return "lower_legs_glutes"
    if "core" in focus or "abs" in focus:
        return "functional_core"
    if "power" in focus:
        return "functional_power"
    if "conditioning" in focus:
        return "functional_conditioning"
    if "chest" in focus or "arms" in focus:
        return "upper_chest_arms"
    return ""


def normalized_request_has_workout_change(normalized: dict[str, Any]) -> bool:
    return bool(
        normalized.get("focus_category")
        or normalized.get("cancel_today")
        or normalized.get("injury_reported")
        or workout_volume_change_present(normalized)
        or str(normalized.get("request_type", "")) in {"workout_change", "mixed_change", "recovery_change"}
    )


def food_matches_any_avoidance(food_name: str, avoidances: list[str]) -> bool:
    normalized_name = normalize_loose(food_name)
    return any(normalize_loose(avoidance) and normalize_loose(avoidance) in normalized_name for avoidance in avoidances)


def replacement_food_for_meal(*, meal: dict[str, Any], avoidances: list[str], used_foods: set[str]) -> dict[str, Any] | None:
    current_food = get_food_by_name(str(meal.get("food_name", "")))
    if not current_food:
        return None
    category = str(current_food.get("category", ""))
    candidates = find_foods(category=category, limit=8)
    for candidate in candidates:
        candidate_name = str(candidate.get("name", "")).strip()
        if not candidate_name or candidate_name in used_foods or food_matches_any_avoidance(candidate_name, avoidances):
            continue
        grams = extract_grams(str(meal.get("serving_size", "100g")), default=100.0)
        macro = calculate_food_macros(str(candidate.get("id", "")), grams)
        return {
            "food_name": candidate_name,
            "serving_size": f"{int(grams)}g",
            "calories": int(round(macro["calories"])),
            "protein_g": macro["protein_g"],
            "carbs_g": macro["carbs_g"],
            "fat_g": macro["fat_g"],
            "meal_slot": str(meal.get("meal_slot", "meal")),
        }
    return None


def extract_grams(serving_size: str, default: float) -> float:
    digits = "".join(char for char in serving_size if char.isdigit() or char == ".")
    return float(digits) if digits else default


def summarize_session_for_chat(session: dict[str, Any]) -> str:
    if not session:
        return "Today is not a scheduled training day."
    if session.get("is_cancelled"):
        return "Today's workout is cancelled by the app safety rules."
    exercises = [f"{exercise.get('name', '')} {exercise.get('sets', '')}x{exercise.get('reps', '')}".strip() for exercise in session.get("exercises", [])]
    exercise_text = ", ".join(exercises) if exercises else "no exercises"
    return f"Today's plan is now {session.get('focus', 'training')}: {exercise_text}."


def select_today_session(workout_sessions: list[dict[str, Any]], reference_date: str) -> dict[str, Any]:
    for session in workout_sessions:
        if same_iso_date(session.get("scheduled_date"), reference_date):
            return session
    return {}


def refresh_youtube_resources(result: FitnessAgentState | dict[str, Any]) -> None:
    exercise_names = [str(exercise.get("name", "")).strip() for session in result.get("current_plan", {}).get("workout_sessions", []) for exercise in session.get("exercises", []) if str(exercise.get("name", "")).strip()]
    result["youtube_resources"] = build_video_resources(exercise_names)


def record_memory_plan_modification(*, session_state: dict[str, Any], date_iso: str, action_type: str, summary: str, injury_areas: list[str] | None = None) -> None:
    store = normalize_memory_store(session_state.get("memory_store"))
    session_state["memory_store"] = append_memory_item(
        store,
        "plan_modification_logs",
        {
            "date": safe_iso_date(date_iso) or date_iso,
            "action_type": action_type,
            "summary": summary,
            "injury_areas": injury_areas or [],
            "recorded_at": datetime.now().isoformat(timespec="seconds"),
        },
    )


def record_memory_injury_event(*, session_state: dict[str, Any], date_iso: str, injury_areas: list[str], source: str, summary: str, risk_level: str = "medium") -> None:
    store = normalize_memory_store(session_state.get("memory_store"))
    safe_date = safe_iso_date(date_iso) or date_iso
    areas = injury_areas or ["reported injury area"]
    for area in areas:
        normalized_area = normalize_loose(area) or "reportedinjuryarea"
        store = append_memory_item(
            store,
            "injury_events",
            {
                "id": f"{safe_date}-{normalized_area}",
                "date": safe_date,
                "area": area,
                "risk_level": risk_level,
                "source": source,
                "summary": summary,
                "status": "active",
                "expires_after_days": 7,
                "recorded_at": datetime.now().isoformat(timespec="seconds"),
            },
            unique_key="id",
        )
    session_state["memory_store"] = store


def record_memory_food_avoidance(*, session_state: dict[str, Any], date_iso: str, avoidances: list[str], scope: str) -> None:
    store = normalize_memory_store(session_state.get("memory_store"))
    for food in avoidances:
        store = append_memory_item(
            store,
            "food_preferences",
            {
                "date": safe_iso_date(date_iso) or date_iso,
                "food": food,
                "preference": "avoid",
                "scope": scope,
                "recorded_at": datetime.now().isoformat(timespec="seconds"),
            },
        )
    session_state["memory_store"] = store




def normalize_loose(value: str) -> str:
    return "".join(character.lower() for character in value if character.isalnum())


def dedupe_preserve_order(values: list[str]) -> list[str]:
    seen: set[str] = set()
    deduped: list[str] = []
    for value in values:
        normalized = normalize_loose(value)
        if not normalized or normalized in seen:
            continue
        seen.add(normalized)
        deduped.append(value)
    return deduped


def append_unique_history_item(history: list[dict], item: dict, date_key: str) -> list[dict]:
    if not item:
        return list(history)
    updated_history = list(history)
    item_date = item.get(date_key)
    if item_date:
        for index, existing in enumerate(updated_history):
            if same_iso_date(existing.get(date_key), item_date) or existing.get(date_key) == item_date:
                updated_history[index] = item
                return updated_history
    updated_history.append(item)
    return updated_history


def clamp_float(value: Any, minimum: float, maximum: float, fallback: float) -> float:
    try:
        numeric = float(value)
    except (TypeError, ValueError):
        return fallback
    return max(minimum, min(maximum, numeric))


def clamp_int(value: Any, minimum: int, maximum: int, fallback: int) -> int:
    try:
        numeric = int(value)
    except (TypeError, ValueError):
        return fallback
    return max(minimum, min(maximum, numeric))


def coerce_string_list(value: Any, fallback: list[str]) -> list[str]:
    if not isinstance(value, list):
        return fallback
    cleaned = [str(item).strip() for item in value if str(item).strip()]
    return cleaned or fallback
